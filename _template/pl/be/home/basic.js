ngApp.provider.controller('ctrlHome', [
  ...
  function (
    ...
    $scope.openMatter = function (matter) {
      location.href =
        (matter.type || matter.matter_type) !== undefined
          ? matter.url
          : '/rest/site/home?site=' + matter.siteid
    }
    $scope.checked = function (index) {
      ...
      if (
        $scope.platform &&
        $scope.platform.home_qrcode_group &&
        $scope.platform.home_qrcode_group.length &&
        $scope.platform.home_qrcode_group[index]
      ) {
    ...
    function listSites() {
      $http
        .get('/rest/home/listSite?page=' + sitePageAt + '&size=10')
    ...
    $scope.listApps = function () {
      $http
        .get('/rest/home/listApp?page=' + appPageAt + '&size=10')
        .success(function (rsp) {
    ...
    $scope.listMatters = function () {
      $http
        .get('/rest/home/listMatter?page=' + matterPageAt + '&size=10')
        .success(function (rsp) {
          if (rsp.data.matters.length) {
    ...
    $scope.$watch('platform', function (platform) {
      ...
      $http
        .get('/rest/home/listMatterTop?page=1&size=3')
        .success(function (rsp) {
