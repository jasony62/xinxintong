...
var ngMod = angular.module('editor.ui.xxt', [
  'ui.bootstrap',
  'url.ui.xxt',
  'paste.ui.xxt',
])
ngMod.controller('tmsEditorController', [
  '$scope',
  'tmsUrl',
  function ($scope, tmsUrl) {
    /* 插入链接 */
      ...
      tmsUrl.fetch().then(function (oResult) {
        ...
        if ((oUrl = oResult.summary)) {
          var text =
            ...
            (oUrl.title || '链接') +
            '</a>'
          $scope.iframeDoc.querySelector('.tms-editor-content').innerHTML +=
            text
        }
      })
    }
...
ngMod.directive('tmsEditor', [
  '$q',
  '$timeout',
  'http2',
  'tmsPaste',
  function ($q, $timeout, http2, tmsPaste) {
    function _calcTextWidth(text) {
      ...
      return {
        height: height,
        width: width,
        charWidth: parseInt(width / text.length),
      }
    }
    /**
     * 根据触屏事件，设置选中的内容
    ...
    function _setSelectionByTouch(oTarget, oTouchTracks) {
      ...
      if (oStartNode) {
        ...
        oParam.startCharAt = parseInt(
          (oTouchTracks.start.x - oStartNode.parentElement.offsetLeft) /
            oParam.text.charWidth
        )
        oParam.endCharAt = parseInt(
          (oTouchTracks.end.x - oStartNode.parentElement.offsetLeft) /
            oParam.text.charWidth
        )
    ...
    return {
      ...
      link: function ($scope, elem, attrs) {
        ...
        iframeHTML += '<style>'
        iframeHTML +=
          'html,body,body>div{width:100%;height:100%;}body{font-size:16px;margin:0}.tms-editor-content:empty::before{color:lightgrey;content:attr(placeholder);}.tms-editor-content img{max-width:100%;}'
        iframeHTML += '</style>'
        iframeHTML += '<body>'
        iframeHTML +=
          '<div class="tms-editor-content " contentEditable="true" placeholder="添加内容...">' +
          $scope.content +
          '</div>'
        ...
        _divContent.addEventListener('paste', function (e) {
          ...
          tmsPaste.onpaste(text, {
            doc: _iframeDoc,
            filter: { whiteSpace: true },
          })
        })
        /* 设置基本样式 */
        $timeout(function () {
          var btns = document.querySelectorAll(
            '#' + $scope.id + ' button[command]'
          )
        ...
        if (window.xxt && window.xxt.image) {
          ...
          eleDivContent = eleIframe.getElementsByClassName(
            'tms-editor-content '
          )[0]
          if (
            (eleBtnInsertImage = document.querySelector(
              '#' + $scope.id + ' button[action=InsertImage]'
            ))
          ) {
            eleBtnInsertImage.addEventListener('click', function () {
              ...
              window.xxt.image.choose($q.defer()).then(function (imgs) {
                imgs.forEach(function (oImg) {
                  window.xxt.image.wxUpload($q.defer(), oImg).then(() => {
                    http2
                      .post(
                        '/rest/site/fe/matter/upload/image?site=platform',
                        oImg
                      )
                      .then(function (rsp) {
                        _iframeDoc.execCommand(
                          'InsertImage',
                          false,
                          rsp.data.url
                        )
