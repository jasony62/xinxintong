...
ngMod.directive('tmsTrace', [
  '$q',
  '$timeout',
  'http2',
  function ($q, $timeout, http2) {
    ...
    var TraceStack = function () {
      ...
      this.pushEvent = function (type, traceBiz, traceText) {
        ...
        if (this.events.length === 0) {
          ...
          if (
            oLastEvent.type !== oNewEvent.type ||
            oNewEvent.elapse - oLastEvent.elapse > EventInterval
          ) {
            this.events.push(oNewEvent)
            storeTrace(this)
          }
        ...
        _timer = $timeout(function () {
          ...
          if (oTraceStack.sendUrl) {
            if ((oStorage = window.localStorage)) {
              ...
              http2.post(
                oTraceStack.sendUrl,
                { start: oCachedTrack.start, events: oCachedTrack.events },
                { showProgress: false }
              )
            }
          }
