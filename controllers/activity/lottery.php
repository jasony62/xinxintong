...
class lottery extends member_base {
     ...
     * $shareby 谁做的分享
     */
    public function index_action($mpid, $lid, $shareby='', $openid=null, $src=null, $code=null, $state=null) 
    {
        empty($mpid) && die('mpid is empty.');
        empty($lid) && die('lid is empty.');

        $model = $this->model('activity/lottery');
        $lot = $model->byId($lid);
        $current = time();
        /**
         * start?
        ...
        if ($code !== null && $state !== null) {
            ...
            $state = json_encode(array($mpid, $lid, $shareby));
            $state = $this->model()->encrypt($state, 'ENCODE', 'lottery');
     ...
     * 
     */
    protected function afterOAuth($state, $who=null, $preactivitydone=false)
    {
        list($mpid, $lid, $shareby) = json_decode($this->model()->encrypt($state, 'DECODE', 'lottery'));
        ...
        if ($lot->fans_enter_only === 'Y')
            $this->askFollow($mpid, $ooid, $osrc);
        /**
         * 访问控制
         */
        if ($lot->access_control === 'Y')
            $this->accessControl($mpid, $lot->lid, $lot->authapis, array($ooid, $osrc), $lot);
        ...
        $params['visitor']['src'] = isset($osrc) ? $osrc : '';
         ...
         * 抽奖活动定义
         */
        $r = $model->byId($lid, 'lid,pic,summary,title,show_greeting,show_winners,autostop,maxstep,chance max_chance', array('award','plate'));
        /**
         * 获得当前用户的抽奖数据
         */
        $r->myAwards = $model->getLog($lid, $mid, $ooid, $osrc);
        $r->chance = $model->getChance($lid, $mid, $ooid, $osrc);
        $params['lottery'] = $r;
    ...
    {
        return $this->model('acl')->canAccessAct($mpid, $lid, 'L', $member, $authapis);
    }
    /**
     * 完成前置活动
     *
     ...
     *
     */
    public function preactiondone_action($mpid, $lid, $code=null, $state=null)
    {
        if ($code !== null) {
            $who = $this->getOAuthUserByCode($mpid, $code);
        } else {
            $shareid = $this->myGetCookie("_{$lid}_shareid");
            $state = json_encode(array($mpid, $lid, $shareid));
            $state = $this->model()->encrypt($state, 'ENCODE', 'lottery');
         ...
         * 记录前置活动执行状态
         */
        $lot = $this->model('activity/lottery')->byId($lid, 'end_at');
        $expire = (int)$lot->end_at;
        $this->mySetCookie("_{$lid}_precondition", 'done', $expire);

        $this->afterOAuth($state, $who, true);
    }
    /**
     * 最近的获奖者清单
     */
    ...
    {
        $winners = $this->model('activity/lottery')->getWinners($lid);
    ...
    public function play_action($mpid, $lid) 
    {
        ...
        if ($r->fans_only === 'Y') {
            ...
            $q = array(
                'count(*)',
                'xxt_fans',
                "mpid='$mpid' and openid='$openid' and src='$src' and unsubscribe_at=0"
            );
         ...
         */
        if ((empty($openid) || empty($src)) && empty($mid))
            return new ComplianceError('无法确定您的身份信息，不能参与抽奖！');
        /**
         * 是否完成了指定内置任务
         */
        if ($task = $model->hasTask($lid, $mid, $openid, $src))
            return new ResponseData(null, 301, $task->description);
        /**
         * 还有参加抽奖的机会吗？
         */
        if (false === $model->canPlay($lid, $mid, $openid, $src, true))
         ...
         * record result
         */
        $model->recordResult($mpid, $lid, $mid, $openid, $src, $selectedAwardID);
        /**
         * 领取非实体奖品
         */
        if ($myAward['type'] == 1 || $myAward['type'] == 2 || $myAward['type'] == 3)
            $model->acceptAward($lid, $mid, $openid, $src, $myAward);
        /**
         * 检查剩余的机会
         */
        $chance = $model->getChance($r->lid, $mid, $openid, $src);
        /**
         * 返回奖项信息
         */
     ...
     * 返回当前用户获得的奖品
     */
    public function myawards_action($lid)
    {
        $model = $this->model('activity/lottery');
        /**
         * 抽奖活动定义
         */
        $r = $model->byId($lid, 'mpid,access_control,authapis', array('award'));
        /**
         * is member? 
         */
        $mid = null;
        if ($r->access_control) {
            $aAuthapis = explode(',', $r->authapis);
            if ($members = $this->getCookieMember($r->mpid, $aAuthapis))
                $mid = $members[0]->mid;
        }
        /**
         * 获得open和src
         */
        if ($fan = $this->myGetcookie("_{$r->mpid}_oauth"))
            $fan = json_decode($this->model()->encrypt($fan,'DECODE',$r->mpid));
        else
            $fan = array(null, null);
        list($openid, $src) = $fan;

        $myAwards = $model->getLog($lid, $mid, $openid, $src, true);
