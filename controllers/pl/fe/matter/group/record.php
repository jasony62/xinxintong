...
class record extends \pl\fe\matter\base
{
   ...
   * 从其他活动导入数据
   */
  public function assocWithApp_action($app, $keepExistent = 'N')
  {
    if (false === $this->accountUser()) {
      return new \ResponseTimeout();
    }
    ...
    if (!empty($oParams->app)) {
      ...
      if ($oParams->appType === 'registration') {
        $oSourceApp = $modelGrpRec->assocWithEnroll($oApp, $oParams->app, $keepExistent === 'Y');
      } else if ($oParams->appType === 'signin') {
        $oSourceApp = $modelGrpRec->assocWithSignin($oApp, $oParams->app, 'Y', $keepExistent === 'Y');
      } else if ($oParams->appType === 'mschema') {
        $oSourceApp = $modelGrpRec->assocWithMschema($oApp, $oParams->app, $keepExistent === 'Y');
      }
    }
  ...
  public function update_action($app, $ek)
  {
    ...
    if (empty($oPosted->role_teams)) {
      ...
      $roleTeams = array_map(function ($oTeam) {
        return $oTeam;
      }, $oPosted->role_teams);
