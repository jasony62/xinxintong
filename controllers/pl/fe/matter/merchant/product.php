...
class product extends \pl\fe\matter\base {
	...
	public function index_action() {
		\TPL::output('/pl/fe/matter/merchant/product/frame');
		exit;
	}
	/*
		*
	...
	*/
	...
	public function get_action($product) {
		$model = $this->model('matter\merchant\product');
		$options = array(
			'cascaded' => 'Y',
		);
	...
	public function cascaded_action($product) {
		$cascaded = $this->model('matter\merchant\product')->cascaded($product);
	 ...
	 * @param int $catelog
	 */
	public function create_action($site, $catelog) {
		...
		$this->model('matter\merchant\property')->referByCatelog($catelog->id);
		/*创建商品*/
		...
		//$cascaded = $this->model('matter\merchant\product')->cascaded($product['id']);
	...
	private function propValueCreate($cateId, $propId, $data) {
		$cate = $this->model('matter\merchant\catelog')->byId($cateId);
		if (false === $cate) {
	...
	public function propUpdate_action($product) {
		...
		$product = $this->model('matter\merchant\product')->byId($product);
		/**
		 * 获得属性值ID
		 */
		$q = array(
	...
	public function skuCreate_action($product, $cateSku) {
		$prod = $this->model('matter\merchant\product')->byId($product);
		$cateSku = $this->model('matter\merchant\catelog')->skuById($cateSku);
		...
		$sku = $this->model('matter\merchant\sku')->byId($skuId);

		/*更新catelog sku状态*/
		$this->model('matter\merchant\catelog')->useSku($cateSku->id);
