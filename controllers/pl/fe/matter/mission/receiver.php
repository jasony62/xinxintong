...
class receiver extends \pl\fe\matter\base {
	...
	public function list_action($site, $app) {
		...
		$modelRev = $this->model('matter\mission\receiver');
	...
	public function remove_action($app, $receiver) {
		...
		/* 记录操作日志 */
		$modelEnl = $this->model('matter\mission');
	...
	public function add_action($site, $app) {
		...
		$modelApp = $this->model('matter\mission');
		...
		foreach ($users as $user) {
			...
			if (empty($modelRev->query_obj_ss(['*', 'xxt_mission_receiver', "siteid='$site' and aid='$app' and userid='$uid'"]))) {
