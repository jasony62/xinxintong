...
class remark extends base
{
  ...
  private function _notifyReceivers($oApp, $oRecord, $oRemark)
  {
    ...
    if (empty($receivers)) {
      ...
      $modelLog->log($oRemark->id, 'enroll:' . $oApp->id . '.remark.notify', 'receivers=0');
      return false;
    }

    $page = empty($oApp->notifyConfig->remark->page) ? 'cowork' : $oApp->notifyConfig->remark->page;
    $noticeURL = $oApp->entryUrl . '&ek=' . $oRecord->enroll_key . '&page=' . $page . '#remark-' . $oRemark->id;
    ...
    $oTmplConfig = $this->model('matter\tmplmsg\config')->getTmplConfig($oApp, $noticeName, $aOptions);
    if ($oTmplConfig[0] === false) {
      $modelLog = $this->model('log');
      $modelLog->log($oRemark->id, 'enroll:' . $oApp->id . '.remark.notify', 'tmplConfig=false');
      return false;
    }
    $oTmplConfig = $oTmplConfig[1];
