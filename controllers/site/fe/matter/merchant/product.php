 ...
 * 商品
 */
class product extends \site\fe\matter\base {
	/**
	 * 进入商品展示页
	 *
	 ...
	 * @return
	 */
	public function index_action($site, $shop, $catelog, $product) {
		//$openid = $this->doAuth($site, $code, $mocker);
		$options = array(
			'fields' => 'title',
			'cascaded' => 'N',
		);
		$page = $this->model('matter\merchant\page')->byType('product', $shop, $catelog, 0, $options);
		if (empty($page)) {
			$this->outputError('指定的页面不存在');
		} else {
			$page = $page[0];
			\TPL::assign('title', $page->title);
			\TPL::output('/site/fe/matter/merchant/product');
			exit;
		}
	}
	/**
	 * 获得商品的页面定义
	 */
	public function pageGet_action($site, $shop, $catelog, $product) {
		// current visitor
		$user = $this->who;
		// page
		$page = $this->model('matter\merchant\page')->byType('product', $shop, $catelog);
		if (empty($page)) {
