...
class main extends \site\fe\matter\base
{
   ...
   * 获得进入规则
   */
  public function entryRule_action($mission)
  {
    $oMission = $this->model('matter\mission')->byId($mission, ['fields' => 'siteid,id,state,title,entry_rule']);
    if (false === $oMission || $oMission->state !== '1') {
      return new \ObjectNotFoundError();
    }

    $result = $this->checkEntryRule($oMission);
      ...
      foreach ($mattersByMis as $oMatter) {
        ...
        switch ($oMatter->type) {
          case 'enroll':
            ...
            $oUserData = $modelEnlUsr->byIdInApp($oMatter, $oUser->uid);
