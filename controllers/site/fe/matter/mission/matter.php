...
class matter extends \site\fe\matter\base
{
   ...
   *
   * @param int $mission 项目id
   * @param string $matterType 资料类型，逗号分隔，支持article，link和channel
   * @param int $channel 资料所属频道
   */
  public function docList_action($mission, $matterType = NULL, $channel = NULL)
  {
    $oMission = $this->model('matter\mission')->byId($mission, ['fields' => 'id,title,summary,pic,state']);
    if (false === $oMission || $oMission->state !== '1') {
      return new \ObjectNotFoundError();
    }

    $aMatterTypes = empty($matterType) ? ['article', 'link', 'channel'] : explode(',', $matterType);
    $aOptions = ['is_public' => 'Y'];
    if (!empty($channel) && intval($channel)) {
      $aOptions['byChannel'] = $channel;
    }

    $docs = $this->model('matter\mission\matter')->byMission($oMission->id, $aMatterTypes, $aOptions);
