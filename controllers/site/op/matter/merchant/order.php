 ...
 * 订单
 */
class order extends \site\op\base {
	/**
	 *
	 */
	...
	public function index_action($shop) {
		...
		$page = $this->model('matter\merchant\page')->byType('op.order', $shop, 0, 0, $options);
		$page = $page[0];

		\TPL::assign('title', $page->title);
		\TPL::output('/site/op/matter/merchant/order');
		exit;
	}
	/**
	 *
	...
	public function pageGet_action($site, $shop) {
		...
		$user = new \stdClass;
		// page
		$page = $this->model('matter\merchant\page')->byType('op.order', $shop);
		if (empty($page)) {
	 ...
	 * 保存订单反馈信息并通知用户
	 */
	public function feedback_action($site, $order) {
		$order = $this->model('matter\merchant\order')->byId($order);
		...
		$this->_notify($site, 'feedback', $order);
	 ...
	 *
	 */
	public function finish_action($site, $order) {
		...
		/*发通知*/
		$order->feedback = json_decode($pv);
		$this->_notify($site, 'finish', $order);
	...
	public function cancel_action($site, $order) {
		...
		/*发通知*/
		$order->feedback = json_decode($pv);
		$this->_notify($site, 'cancel', $order);
	 ...
	 * 通知客户已受理订单
	 */
	private function _notify($site, $action, $order) {
		$modelProd = $this->model('matter\merchant\product');
		...
		foreach ($pendings as $pending) {
			...
			foreach ($datas as $data) {
				/*发送模版消息*/
				$this->tmplmsgSendByOpenid($site, $tmplmsg->id, $order->buyer_openid, $data, $url);
				break;
			}
		}
		return true;
