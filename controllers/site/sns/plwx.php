...
class plwx extends \member_base {
	...
	public function api_action($site) {
		...
		case 'GET':
			/* 公众平台对接 */
			$wxConfig = $this->model('pl\sns\wx')->byPl();
			$wxProxy = $this->model('pl\sns\wx\proxy', $wxConfig);
	...
	private function _subscribeCall($call, $scene_id = null) {
		...
		$wxConfig = $this->model('pl\sns\wx')->byPl();
		$modelFan = $this->model('pl\sns\wx\fan');
		if ($fan = $modelFan->byOpenid($openid, '*')) {
			/**
			 * 粉丝重新关注
			 */
			$modelFan->update(
			...
			$fan = $modelFan->blank($openid, true, array(
				'userid' => $siteUser->uid,
				'subscribe_at' => $current,
				'sync_at' => $current)
			);
		...
		if ($wxConfig->can_fans === 'Y') {
			...
			if ($fanInfo[0]) {
				...
				$this->model()->update('xxt_pl_wxfan', $u, "openid='$openid'");
		...
		if ($reply = $this->model('pl\sns\wx\event')->otherCall($siteId, 'subscribe')) {
	...
	private function _templateCall($call, $msgid, $status) {
		 ...
		 */
		if ($reply = $this->model('pl\sns\wx\event')->otherCall($siteId, 'templatemsg')) {
			$r = $this->model('sns\reply\\' . $reply->matter_type, $call, $reply->matter_id);
			$r->exec();
		}
	}
	...
	private function _cardCall($call) {
		$siteId = $call['siteid'];
		if ($reply = $this->model('pl\sns\wx\event')->otherCall($siteId, 'cardevent')) {
	...
	private function _textCall($call) {
		$siteId = $call['siteid'];
		$text = $call['data'];
		if ($reply = $this->model('pl\sns\wx\event')->textCall($siteId, $text)) {
	...
	private function _menuCall($call, $k) {
		$siteId = $call['siteid'];
		if ($reply = $this->model('pl\sns\wx\event')->menuCall($siteId, $k)) {
	...
	private function _universalCall($call) {
		$siteId = $call['siteid'];
		if ($reply = $this->model('pl\sns\wx\event')->otherCall($siteId, 'universal')) {
			$r = $this->model('sns\reply\\' . $reply->matter_type, $call, $reply->matter_id);
			$r->exec();
		}
	}
	...
	private function _qrcodeCall($call) {
		$siteId = $call['siteid'];
		$data = json_decode($call['data']);
		if ($reply = $this->model('pl\sns\wx\event')->qrcodeCall($siteId, $data[1])) {
