...
class browser extends uploader {
    ...
    public function __construct() 
    {
        ...
        if (isset($this->get['theme']) &&
            ($this->get['theme'] == $this->my_basename($this->get['theme'])) &&
    ...
    {
        if ((!is_dir($this->toLocalEncoding($thumbsDir)) && !$this->mklocaldir($thumbsDir)) ||
    ...
    protected function act_thumb() 
    {
        ...
        $file = $this->toLocalEncoding($this->get['file']);
    ...
    protected function act_newDir() 
    {
        ...
        if (file_exists($this->toLocalEncoding("$dir/$newDir")))
            $this->errorMsg("A file or folder with that name already exists.");
        if (!@mkdir($this->toLocalEncoding("$dir/$newDir"), $this->config['dirPerms']))
    ...
    protected function act_renameDir() 
    {
        ...
        if (is_dir($this->toLocalEncoding($thumbDir)))
    ...
    protected function act_deleteDir() 
    {
        ...
        $result = !dir::prune($this->toLocalEncoding($dir), false);
        if (is_array($result) && count($result))
            $this->errorMsg("Failed to delete {count} files/folders.",
                array('count' => count($result)));
        $thumbDir = "$this->thumbsTypeDir/{$this->post['dir']}";
        if (is_dir($this->toLocalEncoding($thumbDir))) dir::prune($this->toLocalEncoding($thumbDir));
        return true;
    }
    ...
    protected function act_rename() 
    {
        ...
        if (file_exists($thumbFile))
            @rename($thumbFile, "$thumbDir/" . $this->my_basename($newName));
        return true;
    }
    ...
    protected function act_delete()
    {
        ...
        if (!$this->config['access']['files']['delete'] ||
            ...
            !isset($this->post['file']) ||
            (false === ($file = "$dir/{$this->post['file']}")) ||
            !file_exists($this->toLocalEncoding($file)) || !is_readable($this->toLocalEncoding($file)) || !file::isWritable($this->toLocalEncoding($file)) ||
            !@unlink($this->toLocalEncoding($file))
        )
        $this->errorMsg("Unknown error.");

        $thumb = "{$this->thumbsTypeDir}/{$this->post['dir']}/{$this->post['file']}";
        if (file_exists($this->toLocalEncoding($thumb))) @unlink($this->toLocalEncoding($thumb));
        return true;
    }
    //todo not support.
    protected function act_cp_cbd() 
    {
        ...
        if (!$this->config['access']['files']['copy'] ||
            !isset($this->post['dir']) ||
            !is_dir($this->toLocalEncoding($dir)) || !is_readable($this->toLocalEncoding($dir)) || !dir::isWritable($this->toLocalEncoding($dir)) ||
        ...
        foreach($this->post['files'] as $file) {
            ...
            elseif (!@copy($path, "$dir/$base"))
                ...
                if (is_file($fromThumb) && is_readable($fromThumb)) {
                    ...
                    if (!is_dir($this->toLocalEncoding($toThumb)))
    ...
    protected function act_rm_cbd() 
    {
        ...
        foreach($this->post['files'] as $file) {
            ...
            $base = $this->my_basename($file);
            $replace = array('file' => $base);
            if (!is_file($this->toLocalEncoding($path)))
                $error[] = $this->label("The file '{file}' does not exist.", $replace);
            elseif (!@unlink($this->toLocalEncoding($path)))
                $error[] = $this->label("Cannot delete '{file}'.", $replace);
            else {
                $thumb = "{$this->thumbsDir}/$file";
                if (is_file($this->toLocalEncoding($thumb))) @unlink($this->toLocalEncoding($thumb));
            }
        }
        if (count($error))
            return json_encode(array('error' => $error));
    ...
    protected function act_downloadDir() 
    {
        ...
        if (!isset($this->post['dir']) || $this->config['denyZipDownload'])
            $this->errorMsg("Unknown error.");
        $filename = $this->my_basename($dir) . ".zip";
        do {
...
die;
    ...
    protected function act_downloadSelected() 
    {
        ...
        if ($res === TRUE) {
            foreach ($zipFiles as $cfile)
                $zip->addFile($cfile, $this->my_basename($cfile));
            $zip->close();
        }
        header("Content-Type: application/x-zip");
        header('Content-Disposition: attachment; filename="selected_files_' . $this->my_basename($file) . '"');
        header("Content-Length: " . filesize($file));
        readfile($file);
        unlink($file);
        die();
    ...
    protected function act_downloadClipboard() 
    {
        ...
        header("Content-Type: application/x-zip");
        header('Content-Disposition: attachment; filename="clipboard_' . $this->my_basename($file) . '"');
        header("Content-Length: " . filesize($file));
        readfile($file);
        unlink($file);
die;
    ...
    protected function moveUploadFile($file, $dir) 
    {
        ...
        $target = "$dir/".$this->toUTF8(file::getInexistantFilename($this->toLocalEncoding($filename), $this->toLocalEncoding($dir)));

        if (!@move_uploaded_file($file['tmp_name'], $this->toLocalEncoding($target)) &&
            !@rename($file['tmp_name'], $this->toLocalEncoding($target)) &&
            !@copy($file['tmp_name'], $this->toLocalEncoding($target))) 
        {
            @unlink($file['tmp_name']);
            return "{$file['name']}: " . $this->label("Cannot move uploaded file to target folder.");
        } elseif (function_exists('chmod'))
            chmod($this->toLocalEncoding($target), $this->config['filePerms']);
    ...
    protected function getFiles($dir) 
    {
        ...
        $return = array();
        $files = dir::content($this->toLocalEncoding($dir), array('types' => "file"));
        if ($files === false)
            return $return;
        ...
        foreach ($files as $file) {
            ...
            if (is_array($size) && count($size)) {
                $thumb_file = "$thumbDir/" . $this->my_basename($file);
                if (!is_file($this->toLocalEncoding($thumb_file)))
            ...
            $thumb = file_exists($this->toLocalEncoding("$thumbDir")."/$name");
            $return[] = array(
                'name' => stripcslashes($this->toUTF8($name)),
                'size' => $stat['size'],
                'mtime' => $stat['mtime'],
    ...
    protected function getDirs($dir) 
    {
        ...
        if (is_array($dirs)) {
            ...
            foreach ($dirs as $cdir) {
                $info = $this->getDirInfo($cdir, false, true);
    ...
    {
        !$skipEncoding && $dir = $this->toLocalEncoding($dir);

        if ((substr($this->my_basename($dir), 0, 1) == ".") || !is_dir($dir) || !is_readable($dir))
            return false;

        $dirs = dir::content($dir, array('types' => "dir"));
        if (is_array($dirs)) {
            foreach ($dirs as $key => $cdir)
                if (substr($this->my_basename($cdir), 0, 1) == ".")
                    unset($dirs[$key]);
        ...
        $writable = dir::isWritable($dir);
        $info = array(
            'name' => stripslashes($this->my_basename($this->toUTF8($dir))),
