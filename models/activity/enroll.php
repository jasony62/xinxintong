...
class enroll_model extends TMS_MODEL {
    ...
    public function byId($aid, $fields='*') 
    {
         ...
         */
        if ($fields === '*') {
            foreach (array('form','result') as $pname) {
                $page = TMS_APP::model('code/page')->byId($a->{$pname.'_code_id'}, 'html,css,js');
                $page->id = 0;
                $page->name = $pname;
                $page->code_id = $a->{$pname.'_code_id'};
                $a->pages[$pname] = $page;
            }
            $extraPages = TMS_APP::model('activity/enroll')->getPages($aid);
     ...
     * todo
     */
    public function &byPromoter($mpid, $openid, $page=1, $size=10)
    {
        $q = array(
            '*',
            'xxt_activity',
            "mpid='$mpid' and promoter='$openid'"
        );
        $q2 = array(
            'o'=>'create_at desc',
            'r'=>array(
     ...
     * 获得活动的唯一编号
     */
    public function code($mpid)
    {
        $acode = rand(100000,999999);
        $q = array(
            'count(*)',
            'xxt_activity',
            "mpid='$mpid' and code='$acode'"
        );
        while (1===(int)$this->query_val_ss($q)) {
            $acode = rand(100000,999999);
        }
    ...
    public function getPage($aid, $apid)
    {
        ...
        $from = 'xxt_activity_page ap,xxt_code_page cp';
    ...
    public function getPages($aid)
    {
        $select = 'ap.*,cp.html,cp.css,cp.js';
        $from = 'xxt_activity_page ap,xxt_code_page cp';
        $where = "ap.aid='$aid' and ap.code_id=cp.id";

        $q = array($select, $from, $where);

        $pages = $this->query_objs_ss($q);
     ...
     *
     */
    public function addPage($mpid, $aid)
    {
        ...
        $apid = $this->insert('xxt_activity_page', $newPage, true);
    ...
    public function delPage($aid, $apid)
    {
        ...
        $rst = $this->delete('xxt_activity_page', "aid='$aid' and id=$apid");
    ...
    {
        $ek = $this->genEnrollKey($mpid, $act->aid);
        $i = array(
            'aid' => $act->aid,
            'mpid' => $mpid,
            'enroll_at' => time(),
        ...
        if ($activeRound = $this->getActiveRound($mpid, $act->aid))
            $i['rid'] = $activeRound->rid;

        $this->insert('xxt_activity_enroll', $i, false);
    ...
    public function getRecordById($ek)
    {
        $q = array(
            ...
            'xxt_activity_enroll e,xxt_fans f',
            "e.mpid=f.mpid and e.openid=f.openid and e.enroll_key='$ek'"
        );
    ...
    {
        if ($clean)
            $this->delete('xxt_activity_enroll_cusdata', "aid='$aid' and enroll_key='$ek'");
        else
            $fields = $this->query_vals_ss(array('name','xxt_activity_enroll_cusdata', "aid='$aid' and enroll_key='$ek'"));
        ...
        if (!empty($data)) {
            foreach ($data as $n=>$v) {
                ...
                if (is_array($v) && isset($v[0]->imgSrc)) {
                    ...
                    foreach ($v as $img) {
                        $rst = $fsuser->storeImg($runningMpid, $img);
                        if (false === $rst[0])
                            return $rst;
                        $vv[] = $rst[1];
                    }
                     ...
                     * 文本和选择题
                     */
                    $vv = is_string($v) ? mysql_real_escape_string($v) : implode(',', array_keys(array_filter((array)$v,function($i){return $i;})));
                }
                if (!empty($fields) && in_array($n, $fields)) {
                    ...
                    $this->insert('xxt_activity_enroll_cusdata', $ic, false);
                }
            }
            return array(true);
        }
    ...
    public function getRecordRemarks($ek, $page=1, $size=30)
    {
        $q = array(
            'r.*,f.nickname',
            'xxt_activity_enroll e, xxt_activity_enroll_remark r, xxt_fans f',
    ...
    public function getLotteryRoll($aid, $rid) 
    {
        ...
        $q = array(
            ...
            'xxt_activity a,xxt_code_page p',
            "a.aid='$aid' and a.form_code_id=p.id"
        );
        ...
        $w = "aid='$aid'";
        $w .= " and not exists(select 1 from xxt_activity_lottery l where e.enroll_key=l.enroll_key)";
        // todo 逻辑有问题，如果要求既是粉丝又是会员怎么办？
        if ($act->wxyx_only === 'Y') {
            $q = array(
                'e.id,e.enroll_key,f.nickname,e.src,e.openid,e.enroll_at,signin_at,e.tags',
                'xxt_activity_enroll e left join xxt_fans f on e.mpid=f.mpid and e.openid=f.openid and e.src=f.src',
                $w
            );
            ...
            $q = array(
                'e.id,e.enroll_key,e.src,e.openid,e.enroll_at,signin_at,e.tags',
                'xxt_activity_enroll e',
                $w
            );
        }
        ...
        $q = array(
            'l.*,e.enroll_key',
            'xxt_activity_lottery l,xxt_activity_enroll e',
    ...
    public function getLotteryWinners($aid, $rid=null) 
    {
        ...
        $q = array(
            ...
            'xxt_activity_lottery l,xxt_activity_lottery_round r,xxt_activity_enroll e',
    ...
    {
        $this->delete('xxt_activity_enroll_cusdata', "aid='$aid' and enroll_key='$key'"); 

        $rst = $this->delete('xxt_activity_enroll', "aid='$aid' and enroll_key='$key'"); 
    ...
    {
        $this->delete('xxt_activity_enroll_cusdata', "aid='$aid'"); 

        $rst = $this->delete('xxt_activity_enroll', "aid='$aid'"); 
    ...
    public function getSchema($html) 
    {
        ...
        if (preg_match_all('/<(div|li).+?wrap=.+?>.+?<\/(div|li)/i', $html, $wraps)) {
            $wraps = $wraps[0];
            foreach ($wraps as $wrap) {
                $def=array();$inp=array();$title=array();$ngmodel=array();$opval=array();$optit=array();
                if (!preg_match('/<input.+?>/', $wrap, $inp) && !preg_match('/<textarea.+?>/', $wrap, $inp))
                    continue;
                $inp = $inp[0];
    ...
    public function getRecords($mpid, $aid, $options=null) 
    {
        ...
        if ($act->wxyx_only === 'Y') {
            $q = array(
                'e.enroll_key,f.fid,f.nickname,f.openid,f.headimgurl,e.enroll_at,signin_at,e.tags,e.score,s.score myscore',
                "xxt_activity_enroll e left join xxt_fans f on e.mpid=f.mpid and e.openid=f.openid left join xxt_activity_enroll_score s on s.enroll_key=e.enroll_key and s.openid='$visitor'",
                $w
            );
        } else if ($act->fans_only === 'Y') {
            $q = array(
            ...
            $q = array(
                'e.enroll_key,m.name,m.mobile,e.enroll_at,e.signin_at,e.tags,e.score,m.mid',
                "xxt_activity_enroll e left join xxt_member m on m.forbidden='N' and e.mid=m.mid",
                $w
            );
        } else {
            $q = array(
                'e.enroll_key,e.enroll_at,e.signin_at,e.score,e.tags',
                'xxt_activity_enroll e',
                $w
            );
        }
