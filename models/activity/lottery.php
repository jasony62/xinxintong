...
class lottery_model extends TMS_MODEL {
    ...
    {
        $q = array($fields, 'xxt_lottery', "lid='$lid'");
     ...
     * 还有多少次参与机会
     */
    public function getChance($lid, $mid, $openid, $src)
    {
        /**
         * 获得抽奖设置
         */
        ...
        if (empty($mid) && (empty($openid) || empty($src)))
        ...
        if (!empty($mid))
            ...
            $q[2] = "lid='$lid' and openid='$openid' and src='$src' and last='Y'";
     ...
     * 如果有返回任务的定义
     */
    public function hasTask($lid, $mid, $openid, $src) 
    {
        ...
        if ($tasklog = $this->query_objs_ss($q, $q2)) {
            ...
            $q = array(
                'count(*)',
                'xxt_shareaction_log',
                "ooid='$openid' and osrc='$src' and (share_to='F' or share_to='T') and matter_type='lottery' and matter_id='$lid' and share_at>$tasklog->create_at"
            );
            if (3 <= (int)$this->query_val_ss($q)) {
                ...
                $this->earnPlayAgain($lid, $mid, $openid, $src, $award);
                /**
                 * 修改任务状态
                 */
                $this->update(
     ...
     */
    public function canPlay($lid, $mid, $openid, $src, $autoUpdateState=false) 
    {
        /**
         * 最近一次抽奖情况
         */
                ...
                ($nowdate['yday'] - $lastdate['yday']) > 0)
            {
                ...
                if ($autoUpdateState) {
                    if (!empty($mid))
                        ...
                        $w = "lid='$lid' and openid='$openid' and src='$src' and last='Y'";
     ...
     * 获得抽奖记录
     */
    public function &getLog($lid, $mid, $openid, $src, $includeAll=false, $page=1, $size=20)
    {
        if (empty($mid) && (empty($openid) || empty($src))) {
            $log = array();
        } else {
            ...
            if (!empty($mid))
                ...
                $q[2] =  "l.lid='$lid' and l.openid='$openid' and l.src='$src' and l.aid=a.aid";

            if (!$includeAll) $q[2] .=  " and a.type!=0";
     ...
     */
    public function recordResult($mpid, $lid, $mid, $openid, $src, $aid) 
    {
        if (empty($mpid))
            $mpid = $this->query_value('mpid', 'xxt_lottery', "lid='$lid'");
        /**
         * 获得之前的签到情况
         */
        $q = array(
     ...
     * 实物奖品：需要在线下进行兑奖
     */
    public function acceptAward($lid, $mid, $openid, $src, $award) 
    {
        $takeaway = false;
        switch ($award['type']) {
        case 1: // 积分
            $takeaway = true;
            $this->earnCredits($lid, $mid, $openid, $src, $award);
            break;
        case 2: // 再来一次
            $takeaway = true;
            $this->earnPlayAgain($lid, $mid, $openid, $src, $award);
        ...
        case 3: // 完成任务
            $takeaway = true;
            $this->earnTask($lid, $mid, $openid, $src, $award);
            break; 
        }
        /**
         * 更新抽奖状态
     ...
     * 获得积分奖励
     */
    public function earnCredits($lid, $mid, $openid, $src, &$award) 
    {
     ...
     * 将累计的抽奖次数减1
     */
    public function earnPlayAgain($lid, $mid, $openid, $src, &$award)
    {
        if (!empty($mid))
            $w = "lid='$lid' and mid='$mid' and last='Y'";
        else
     ...
     * 一个抽奖，一个用户，只允许生成一次任务
     */
    public function earnTask($lid, $mid, $openid, $src, &$award) 
    {
        /**
         * 检查是否已经生成过任务
         */
        ...
        //);
        //if (!empty($openid) && !empty($src))
        //    $q[2] = "lid='$lid' and openid='$openid' and src='$src'";
        //else if (!empty($mid))
    ...
    public function &forCustomPush($runningMpid, $id) 
    {
        ...
        $lot->type = 'lottery';
        $ml[] = array(
            'title'=>urlencode($lot->title),
            'description'=>urlencode($lot->summary),
            'url'=>TMS_APP::model('reply')->getMatterUrl($runningMpid, $lot),
            'picurl'=>urlencode($lot->pic)
        );
