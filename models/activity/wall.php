 ...
 *
 */
class wall_model extends TMS_MODEL {
    /**
     * 审核状态
     */
     ...
     * $cascaded array []
     */
    public function byId($wid, $fields='*', $cascaded=array()) 
    {
        $q = array(
            $fields, 
            'xxt_wall', 
     ...
     */
    public function join($runningMpid, $wid, $openid, $src, $remark=null)
    {
        /**
         * 加入一个信息墙需要从其他的墙退出
         */
        $this->update(
            'xxt_wall_enroll', 
            array('close_at'=>time()), 
            "mpid='$runningMpid' and openid='$openid' and src='$src'"
        );
        /**
         * 加入一个组
         */
        $q = array(
            'count(*)', 
            'xxt_wall_enroll',
            "mpid='$runningMpid' and wid='$wid' and openid='$openid' and src='$src'"
        );
     ...
     * 返回当前用户所在的信息墙
     */
    public function joined($runningMpid, $openid, $src) 
    {
        $q = array(
            'wid', 
            'xxt_wall_enroll',
            "mpid='$runningMpid' and openid='$openid' and src='$src' and close_at=0"
        );
     ...
     * 判断当前用户是否已经参加了指定讨论组
     */
    public function joinedWall($runningMpid, $wid, $openid, $src) 
    {
        $q = array(
            'count(*)', 
            'xxt_wall_enroll',
            "mpid='$runningMpid' and wid='$wid' and openid='$openid' and src='$src' and close_at=0"
        );
    ...
    public function messages($runningMpid, $wid, $page=1, $size=30, $contain=null)
    {
        $q = array(
            ...
            "w.wid=l.wid and l.wid= '$wid' and f.mpid='$runningMpid' and l.openid=f.openid and l.src=f.src"
        );
     ...
     * 获得墙内的所有用户
     */
    public function joinedUsers($runningMpid, $wid, $fields='openid,src')
    {
        $q = array(
            $fields,
            'xxt_wall_enroll',
    ...
    public function pendingMessages($runningMpid, $wid, $time=0)
    {
        ...
        $w = "w.wid=l.wid and f.mpid='$runningMpid' and l.openid=f.openid and l.src=f.src";
     ...
     * 退出信息墙
     */
    public function unjoin($runningMpid, $wid, $openid, $src) 
    {
    ...
    private function add($wid, $msg, $ctrl=null)
    {
        ...
        if ($msg['type'] === 'text' && $msg['data'] === $wall->quit_cmd) {
            ...
            $this->update(
                'xxt_wall_enroll',
                array('close_at'=>time()),
                "wid= '$wid' and openid='$openid' and src='$src'"
            );
            $reply = empty($wall->quit_reply) ? '您已退出信息墙' : $wall->quit_reply;
        ...
        if ($ctrl && 'Y' === $wall->skip_approve) {
            ...
            if ('Y' === $wall->push_others)
                $this->push_others($mpid, $openid, $src, $msg, $wall, $wid, $ctrl);
        }
        /**
         * 记录留言
         */
        ...
        $this->update(
            ...
            "mpid='$mpid' and openid='$openid' and src='$src'"
        );
     ...
     * $wall
     */
    public function push_others($mpid, $openid, $mpsrc, $msg, $wall, $wid, $ctrl) 
    {
        /**
         * 获得当前用户的信息
         */
        $member = TMS_APP::model('user/fans')->byOpenid($mpid, $openid, 'nickname');
        /**
         * 拼装推送消息
         */
        switch ($msg['type']) {
        ...
        if ($mpsrc === 'qy') {
             ...
             */
            $groupUsers = TMS_APP::model('acl')->wallUsers($mpid, $wid);
            if (!empty($groupUsers)) {
                /**
                 * 不推送给发送人
                 */
                ...
                foreach ($ingroup as $ig) {
                    if ($openid === $ig->openid && $mpsrc === $ig->src)
                        continue;
                    $userid = $ig->openid;
                ...
                if (!empty($groupUsers)) {
                    ...
                    if ($msg['type'] === 'text') {
                        $joinUrl = 'http://'.$_SERVER['HTTP_HOST']."/rest/activity/discuss?wid=$wid";
        ...
        if (!$finished) {
            ...
            foreach ($users as $user) {
                if ($openid === $user->openid && $mpsrc === $user->src)
                    continue;
                $ctrl->send_to_user($mpid, $user->src, $user->openid, $message);
            }
        }
