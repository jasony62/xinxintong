...
class token_model extends \TMS_MODEL
{
  ...
  public function token($secret, &$invoke, $expiresIn = 7200)
  {
    ...
    if ($count > 1) { // access_token数据异常存在多条数据，需要清理，并重新生成
      ...
      $invToken = $this->createToken($secret, $invoke, $expiresIn);
    ...
    empty($invToken->expires_in) && $invToken->expires_in = (int) $invToken->expire_at - $current;
  ...
  /*
	 * 创建sccess_token
	 */
  private function createToken($secret, $invoke, $expiresIn)
  {
    $this->setOnlyWriteDbConn(true);
    $current = time();
    ...
    $data->expire_at = $current + $expiresIn;
