...
class data_model extends entity_model
{
        ...
        switch ($oSchema->type) {
              ...
              foreach ($submitVal as $oFile) {
                if (isset($oFile->uniqueIdentifier)) {
                  if (is_numeric(TMS_UPLOAD_FILE_MAXSIZE) && TMS_UPLOAD_FILE_MAXSIZE > 0) {
                    if ($oFile->size > TMS_UPLOAD_FILE_MAXSIZE * 1024 * 1024) {
          ...
          case 'multiple':
            if (is_object($submitVal)) {
              // 多选题，将选项合并为逗号分隔的字符串
              $treatedValue = implode(',', array_keys(array_filter((array) $submitVal, function ($i) {
                return $i;
              })));
              $oDbData->{$schemaId} = $treatedValue;
            } else if (is_string($submitVal)) {
      ...
      switch ($oSchema->type) {
        ...
        default: // 主观题
          if ($oSchema->type === 'voice') {
            $userAnswer = isset($treatedValue[0]->text) ? $treatedValue[0]->text : '';
          } else {
          ...
          if (!empty($oAssignScore) && isset($oAssignScore->{$oSchema->id})) {
            ...
            if (!empty($oLastSchemaDatas) && count($oLastSchemaDatas) === 1 && !empty($oLastSchemaDatas[0]->score)) {
              ...
              if ($oSchema->type === 'voice') {
                ...
                if ($oLastUserAnswer) {
                  $lastUserAnswer = isset($oLastUserAnswer[0]->text) ? $oLastUserAnswer[0]->text : '';
                }
              } else {
  ...
  {
    // 设置了可见性规则的题目
    $visibilitySchemas = array_filter($oApp->dynaDataSchemas, function ($oSchema) {
      return !empty($oSchema->visibility->rules);
    });
    // 关联的分组题
