...
class formula {
    ...
    public static function calculate($exp, $exp_values) {
        ...
        $output_queue = self::_nifix2rpn($exp_arr);
        return self::_calculate_value($output_queue, $exp_values);
    }
    ...
    //将中缀表达式转为后缀表达式
    protected static function _nifix2rpn($input_queue) {
        $exp_stack = [];
        $output_queue = [];
    ...
    protected static function _calculate_value($output_queue, $exp_values) {
        $res_stack = [];
    ...
    public static function calcAndReplaceInnerFunc($equation, $oSchema, $oContext) {
        ...
        /* 当前记录指定题目的数值 */
        if (isset($oContext->data)) {
            $matches = [];
            if (preg_match_all('/data\((.+?)\)/i', $equation, $matches, PREG_SET_ORDER)) {
                $search = [];
                $replace = [];
                ...
                $equation = str_replace($search, $replace, $equation);
            }
        }
