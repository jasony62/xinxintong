    ...
    $fnMakeDynaSchemaByData = function ($oSchema, $oDsAppRnd, $schemaIndex, &$dynaSchemasByIndex, $oTask = null) {
      ...
      foreach ($targetSchemas as $oTargetSchema) {
        ...
        if (count($datas)) {
          ...
          $aRecordCache = [];
          // 表示记录的题目
          $aLabelSchemas = $this->asAssoc($oTargetApp->dynaDataSchemas, ['filter' => function ($oDynaSchema) {
            return $oDynaSchema->type !== 'multitext' && $this->getDeepValue($oDynaSchema, 'shareable') === 'Y';
          }]);
          foreach ($datas as $index => $oRecData) {
