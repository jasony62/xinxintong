		...
		if (($oMission = $this->query_obj_ss($q))) {
			...
			if (!empty($cascaded)) {
				...
				foreach ($cascaded as $field) {
					if ($field === 'header_page_name' && $oMission->header_page_name) {
						$oMission->header_page = $modelCode->lastPublishedByName($oMission->siteid, $oMission->header_page_name, array('fields' => 'id,html,css,js'));
					} else if ($field === 'footer_page_name' && $oMission->footer_page_name) {
						$oMission->footer_page = $modelCode->lastPublishedByName($oMission->siteid, $oMission->footer_page_name, array('fields' => 'id,html,css,js'));
					} else if ($field === 'phase') {
