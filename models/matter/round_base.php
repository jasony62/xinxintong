...
trait Round
{
   ...
   * 有效的定时规则
   */
  public function getEnabledRules($roundCron)
  {
    if (empty($roundCron) || !is_array($roundCron)) {
      return false;
    }
    $enabledRules = array_filter(
      $roundCron,
      function ($oRule) {
        return $this->getDeepValue($oRule, 'enabled') === 'Y' && $this->getDeepValue($oRule, 'purpose') === 'C' && ((int) $this->getDeepValue($oRule, 'stop_at', 0) === 0 || (int) $this->getDeepValue($oRule, 'stop_at', 0) > time());
      }
    );
    return $enabledRules;
  }
    ...
    foreach ($rules as $oRule) {
      if ($oRule->pattern === 'period') {
        switch ($oRule->period) {
          ...
          case 'M':
            if (empty($oRule->mday)) {
              return [false, '请设置定时轮次每月的开始日期！'];
            }
            //if (empty($oRule->end_mday)) {return [false, '请设置定时轮次每月的结束日期！'];}
            if (empty($oRule->hour)) {
              return [false, '请设置定时轮次每月开始日期的几点开始！'];
            }
            break;
            // 0-6 周几
          case 'W':
            if (!isset($oRule->wday)) {
              return [false, '请设置定时轮次每周几开始！'];
            }
            //if (!isset($oRule->end_wday)) {return [false, '请设置定时轮次每周几结束！'];}
            if (empty($oRule->hour)) {
              return [false, '请设置定时轮次每周几的几点开始！'];
            }
            break;
            // 0-23 几点
          default:
            if (empty($oRule->hour)) {
              return [false, '请设置定时轮次每天的几点开始！'];
            }
