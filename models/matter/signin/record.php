...
class record_model extends \matter\enroll\record_base
{
  ...
  public function absentByApp($oApp, $rid = '')
  {
    ...
    if (!empty($oApp->entryRule->group->id)) {
      ...
      $aAllUsrs = array_filter($oGrpUsrsResult->records, function ($oGrpRec) use ($oSigninedUsers) {
        return false === in_array($oGrpRec->userid, $oSigninedUsers);
      });
    } else if (!empty($oApp->entryRule->enroll->id)) {
      $modelRec = $this->model('matter\enroll\record');
      $oResult = $modelRec->byApp($oApp->entryRule->enroll->id);
      $aAllUsrs = array_filter($oResult->records, function ($oRec) use ($oSigninedUsers) {
        return false === in_array($oRec->userid, $oSigninedUsers);
      });
