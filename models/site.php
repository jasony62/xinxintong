...
class site_model extends \TMS_MODEL
{
  ...
  public function byId($siteId, $options = [])
  {
    ...
    if (($site = $this->query_obj_ss($q))) {
      if (property_exists($site, 'config')) {
        $site->config = empty($site->config) ? new \stdClass : json_decode($site->config);
      }
      if (!empty($cascaded)) {
  ...
  public function byFriend($friendIds, $options = [])
  {
    ...
    is_string($friendIds) && $friendIds = explode(',', $friendIds);
  ...
  private function _buildPublishNotice($tmplConfig, $oFromSite, $oMatter)
  {
    ...
    foreach ($tmplConfig->tmplmsg->params as $param) {
      ...
      if ($mapping->src === 'matter') {
        if (isset($oFromSite->{$mapping->id})) {
          $value = $oFromSite->{$mapping->id};
        }
