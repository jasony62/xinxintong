...
class way_model extends \TMS_MODEL
{
   ...
   */
  private function _bindSiteSnsUser($siteId, $snsName, $snsUser, $oCookieUser, $oCookieRegUser = null)
  {
    $modelSiteUser = $this->model('site\user\account');
    ...
    if ($oCookieRegUser) {
      ...
      if (empty($oSiteUser->{$snsName . '_openid'})) {
        ...
        throw new \SiteUserException('数据错误，注册主访客账号已经绑定其他公众号用户', $oSiteUser->uid, $snsUser->openid);
      }
    } else {
