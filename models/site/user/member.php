...
class member_model extends \TMS_MODEL
{
   ...
   */
  public function syncToGroupUser($schemaId, $oNewMember)
  {
    ...
    foreach ($groups as $group) {
      ...
      /* 是否同步到指定分组 */
      $assignTeam = null;
      if (isset($group->syncRule) && !empty($group->syncRule->team_id)) {
        $team = $modelTeam->byId($group->syncRule->team_id, ['fields' => 'team_id,title']);
        if ($team !== false) {
          $assignTeam = new \stdClass;
          $assignTeam->team_id = $team->team_id;
          $assignTeam->title = $this->escape($team->title);
        }
      }
      $users = [$user];
      $cnt = $modelGrpUsr->syncRecord($group->siteid, $group, $users, $this, 'mschema', $assignTeam);
