...
class tag_model extends TMS_MODEL {
     ...
     * $limit
     */
    public function get_tags($mpid, $resType='article', $tags = null, $offset = 0, $limit = false) 
    {
        $resTable = $this->resTable($resType);
        $sql = 'select t.id,t.title';
        $sql .= ',count(t.id) weight'; 
        $sql .= " from xxt_tag t,$resTable r";
        $sql .= " where t.mpid='$mpid' and t.mpid=r.mpid and t.id=r.tag_id";
        $sql .= ' group by t.id';
        $sql .= ' order by weight desc';
        if ($limit)
            $sql .= " limit $offset,$limit";

        $matched_tags = parent::query_objs($sql);
