...
if (!mysql_query($sql)) {
    ...
    echo 'database error(xxt_log): '.mysql_error();
}
/**
 * log received parsed msessages.
 * 这应该是一个全局日志，所有收到的用户消息都要记录
 */
$sql = "create table if not exists xxt_mpreceive_log(";
$sql .= 'id int not null auto_increment';
$sql .= ',mpid varchar(32) not null';
...
if (!mysql_query($sql)) {
    header('HTTP/1.0 500 Internal Server Error');
    echo 'database error(xxt_mpreceive_log): '.mysql_error();
}
/**
 * log send parsed msessages.
 * 这应该是一个全局日志，所有发给用户的消息都要记录
 */
$sql = "create table if not exists xxt_mpsend_log(";
$sql .= 'id int not null auto_increment';
$sql .= ',mpid varchar(32) not null';
$sql .= ',openid varchar(255) not null';
...
if (!mysql_query($sql)) {
    header('HTTP/1.0 500 Internal Server Error');
    echo 'database error(xxt_mpsend_log): '.mysql_error();
}
/**
 * 记录图文信息打开的情况
 */
$sql = "create table if not exists xxt_matter_read_log(";
$sql .= 'id int not null auto_increment';
$sql .= ',vid varchar(32) not null';
$sql .= ',osrc char(2) not null'; // OAuth src 
$sql .= ',ooid varchar(255) not null'; // OAuth openid
$sql .= ',read_at int not null';
$sql .= ',mpid varchar(32) not null';
$sql .= ',matter_id varchar(128) not null';
$sql .= ',matter_type varchar(10) not null'; //article,link 
 ...
 */
$sql = "create table if not exists xxt_shareaction_log(";
$sql .= 'id int not null auto_increment';
...
$sql .= ',mpid varchar(32) not null';
$sql .= ',matter_id varchar(128) not null';
$sql .= ',matter_type varchar(10) not null'; 
 ...
 * 记录模板消息发送情况
 */
$sql = "create table if not exists xxt_tmplmsg_log(";
$sql .= 'id int not null auto_increment';
$sql .= ',mpid varchar(32) not null';
 ...
 * 
 * 阅读：xxt_matter_read_log
 * 分享：xxt_shareaction_log
 */
$sql = "create table if not exists xxt_user_action_log(";
$sql .= 'id int not null auto_increment';
$sql .= ',mpid varchar(32) not null';
$sql .= ',vid varchar(32) not null';
$sql .= ',src char(2) not null'; 
 ...
 */
$sql = "create table if not exists xxt_matter_action_log(";
$sql .= 'id int not null auto_increment';
$sql .= ',mpid varchar(32) not null';
$sql .= ',matter_id varchar(128) not null';
$sql .= ',matter_type varchar(10) not null'; 
