...
$sqls[] = "INSERT INTO xxt_site_admin(siteid,uid,ulabel,urole,creater,creater_name,create_at) SELECT id,creater,creater_name,'O',creater,creater_name,create_at FROM xxt_site s where not exists(select 1 from xxt_site_admin a where s.id=a.siteid and s.creater=a.uid)";
