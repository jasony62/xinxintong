 ...
 * 从数据库中获得数据
 */
$oEnlApps = $model->query_objs_ss(['id,data_schemas', 'xxt_enroll', ['data_schemas' => (object) ['op' => 'like', 'pat' => '%:"member"%']]]);
/**
 * 处理获得的数据
 */
...
$oEnlPages = $model->query_objs_ss(['id,data_schemas', 'xxt_enroll_page', ['data_schemas' => (object) ['op' => 'like', 'pat' => '%:"member"%']]]);
/**
 * 处理获得的数据
 */
foreach ($oEnlPages as $oEnlPage) {
	...
	foreach ($dataSchemas as $oSchemaWrap) {
		if (isset($oSchemaWrap->schema->type) && $oSchemaWrap->schema->type === 'member') {
			$oSchemaWrap->schema->type = 'shorttext';
		}
	}
	$sDataSchemas = $model->escape($model->toJson($dataSchemas));
 ...
 * 从数据库中获得数据
 */
$oSigApps = $model->query_objs_ss(['id,data_schemas', 'xxt_signin', ['data_schemas' => (object) ['op' => 'like', 'pat' => '%:"member"%']]]);
/**
 * 处理获得的数据
 */
 ...
 * 从数据库中获得数据
 */
$oSigPages = $model->query_objs_ss(['id,data_schemas', 'xxt_signin_page', ['data_schemas' => (object) ['op' => 'like', 'pat' => '%:"member"%']]]);
/**
 * 处理获得的数据
 */
 ...
 * 从数据库中获得数据
 */
$oGrpApps = $model->query_objs_ss(['id,data_schemas', 'xxt_group', ['data_schemas' => (object) ['op' => 'like', 'pat' => '%:"member"%']]]);
/**
 * 处理获得的数据
 */
