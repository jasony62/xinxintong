...
tinymce.PluginManager.add('toemaillink', function(editor) {
	...
	function showDialog(linkList) {
		...
		var win, onlyText, textListCtrl, linkListCtrl, relListCtrl, targetListCtrl, codeListCtrl, classListCtrl;
        ...
        data.target = anchorElm ? dom.getAttrib(anchorElm, 'target') : (editor.settings.default_link_target || '_email');
		...
		if (editor.settings.target_list !== false) {
			targetListCtrl = {
				...
				label: 'Target',
                disabled: true,
                values: buildValues('target_list', 'target', [{text:'发送邮件', value:'_email'}, {text:'None', value:''}, {text:'New window', value:'_blank'}]),
                onselect: function() {
                    var code = this.parent().parent().find('#code')[0];
                    if (this.value() == '_email') {
                        code.disabled(false);
                    } else {
    ...
    editor.addMenuItem('toemaillink', {
		icon: 'link',
		text: 'Insert link',
		shortcut: 'Ctrl+K',
		onclick: createLinkList(showDialog),
