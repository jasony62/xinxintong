    ...
    open = function (siteid, options) {
      modalInstance = $uibModal.open({
        ...
        resolve: {
          url: function () {
            return (
              '/kcfinder/browse.php?lang=zh-cn&type=' +
              options.mediaType +
              '&site=' +
              siteid
            )
        ...
        if (!$scope.qrcodeShown) {
          ...
          http2.get(url, function (rsp) {
            ...
            $popover.popover({
              html: true,
              title:
                '<span>扫描发送到手机</span><button class="close" onclick="$popover.popover(\'destroy\')"><span>&times;</span></button>',
            ...
            .open({
              ...
              controller: [
                ...
                function (http2, $scope, $mi, $q) {
                  ...
                  var url =
                      '/rest/pl/fe/site/setting/notice/get?site=' +
                      siteId +
                      '&name=site.matter.push&cascaded=Y',
                    msgMatter = {},
                    urlMatterTypes = [],
                    missionId
                    ...
                    http2
                      ...
                      .then(
                        function (rsp) {
                          ...
                          if (options.sender) {
                            getLastNotice(options.sender).then(function (
                              lastNotice
                            ) {
                              var i, matter
                  ...
                  $scope.ok = function () {
                    var notify,
                      matterType = msgMatter.matterType
                    if (matterType.value === 'tmplmsg') {
                      notify = {
