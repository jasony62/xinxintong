...
controller('WallCtrl',['$scope','$http',function($scope,$http){
    ...
    $scope.$watch('jsonParams', function(nv){
        ...
        $http.get('/rest/activity/discuss/wall?mpid='+$scope.mpid+'&wid='+$scope.wid+'&_='+(new Date()).getTime(), {
            headers: {'Accept':'application/json'}                     
        }).success(function(rsp){
            $scope.messages = rsp.data[0];
            last = rsp.data[1];
            var worker = new Worker("/views/default/activity/discuss/wallMessages.js?v=1");
