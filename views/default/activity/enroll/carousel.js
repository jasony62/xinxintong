...
.controller('ctrl',['$scope','$http','$timeout','$interval',function($scope,$http,$timeout,$interval){
    ...
    $scope.stop = function(){
        var getWinner = function() {
            ...
            $http.post('/rest/mp/activity/enroll/lottery?aid='+$scope.aid+'&rid='+$scope.round+'&ek='+winner.enroll_key, {openid:winner.openid,src:winner.src})
    ...
    $scope.getPersons = function() {
        $http.get('/rest/mp/activity/enroll/lotteryRoll?aid='+$scope.aid+'&rid='+$scope.round).
        success(function(rsp){
    ...
    $scope.changeRound = function() {
        var url = '/rest/activity/enroll/lottery2?aid='+$scope.aid+'&_='+(new Date()).getTime();
        if ($scope.round && $scope.round.length > 0) url += '&rid='+$scope.round;
        location.href = url;
    };
    $scope.cleanAll = function() {
        $http.get('/rest/mp/activity/enroll/lotteryClean?aid='+$scope.aid+'&rid='+$scope.round).
        success(function(rsp){
            $scope.changeRound();
        });
    };
    $scope.$watch('aid', function(nv){
        $http.get('/rest/mp/activity/enroll/lotteryRounds?aid='+$scope.aid).
        success(function(rsp){
