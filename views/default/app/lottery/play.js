...
controller('lotCtrl',['$scope','$http','$timeout',function($scope,$http,$timeout) {
    ...
    var logShare = function(shareto) {
        ...
        url += "&id="+$scope.params.lottery.id; 
        url += "&type=lottery"; 
    ...
    $scope.$watch('jsonParams', function(nv){
        if (nv && nv.length) {
            ...
            var sharelink = 'http://'+location.hostname+"/rest/app/lottery";
            sharelink += "?mpid="+params.mpid;
            sharelink += "&lid="+params.lottery.id;
            ...
            if (params.lottery.show_winners==='Y') {
                $http.get('/rest/app/lottery/winners?lid='+params.lottery.id+'&_='+t, {
    ...
    $scope.play = function(cbSuccess, cbError) {
        $http.get('/rest/app/lottery/play?mpid='+$scope.params.mpid+'&lid='+$scope.params.lottery.id+'&_='+t, {headers:{'Accept':'application/json'}}).success(function(rsp){
    ...
    $scope.debugReset = function() {
        ...
        c = 'xxt_' + $scope.params.lottery.id + '_precondition'+ "=; expires=" + expdate.toGMTString() + "; path=/"; 
        document.cookie = c;
        alert('clean');
    };
}]);
