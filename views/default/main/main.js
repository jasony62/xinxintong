...
controller('MpCtrl',['$scope','http2',function($scope,http2){
    ...
    $scope.createpmp = function() {
        http2.get('/rest/main/createmp?asparent=Y', function(rsp){
            location.replace('/rest/mp/mpaccount?mpid=' + rsp.data);
        });
    };
    $scope.createmp = function() {
        var url = '/rest/main/createmp';
    ...
    $scope.getmps = function() {
        var url = '/rest/main/mpaccounts?_='+t;
        $scope.selectedPmp && (url += '&pmpid='+$scope.selectedPmp.mpid);
        http2.get(url, function(rsp){
            $scope.mps = rsp.data;
        });
    ...
    $scope.removemp = function(event, mp) {
        ...
        http2.get('/rest/main/removemp?mpid='+mp.mpid, function(rsp){
            if (rsp.err_code != 0) {
    ...
    $scope.getmps();
    http2.get('/rest/main/mpaccounts?asparent=Y&_='+t, function(rsp){
        $scope.pmps = rsp.data;
    });
}]);
