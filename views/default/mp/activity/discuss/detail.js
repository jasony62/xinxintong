...
controller('WallCtrl',['$scope','$http','$location','http2',function($scope,$http,$location,http2) {
    ...
    $scope.back = function() {
        location.href = '/page/mp/activity/discuss/main';
    };
    $scope.$watch('subPage', function(nv){
        $scope.$broadcast('changeSubPage');
    });
    http2.get('/rest/mp/activity/discuss?wid='+$scope.wid, function(rsp){
        $scope.wall = rsp.data;
        $scope.picGalleryUrl = '/kcfinder/browse.php?lang=zh-cn&type=图片&mpid='+$scope.wall.mpid;
    });
}]).
controller('settingCtrl',['$scope','http2',function($scope,http2) {
    $scope.update = function(name) {
        ...
        http2.post('/rest/mp/activity/discuss/update?wid='+$scope.wid, nv, function(rsp){
        });
    };
    $scope.setPic = function(){
        $scope.$broadcast('picgallery.open', function(url){
            var t=(new Date()).getTime(),url=url+'?_='+t,nv={pic:url};
            http2.post('/rest/mp/activity/discuss/update?wid='+$scope.wid, nv, function() {
                $scope.wall.pic = url;
            });
        }, false);
    }; 
...
controller('ApproveCtrl',['$scope','http2',function($scope,http2) {
    ...
    $scope.messages = [];
    var worker = new Worker('/views/default/mp/activity/discuss/wallMessages.js?_=1');
    ...
    $scope.approve = function(msg) {
        http2.get('/rest/mp/activity/discuss/approve?wid='+$scope.wid+'&id='+msg.id, function(rsp){
            var i = $scope.messages.indexOf(msg);
            $scope.messages.splice(i,1);
        });
    };
    $scope.reject = function(msg) {
        http2.get('/rest/mp/activity/discuss/reject?wid='+$scope.wid+'&id='+msg.id, function(rsp){
            var i = $scope.messages.indexOf(msg);
            $scope.messages.splice(i,1);
        });
    };
...
controller('usersCtrl',['$rootScope','$scope','http2',function($rootScope,$scope,http2) {
    $scope.doSearch = function() {
        http2.get('/rest/mp/activity/discuss/users?wid='+$scope.wid, function(rsp){
    ...
    $scope.quitWall = function() {
        ...
        if (vcode === $scope.wall.title) {
            http2.get('/rest/mp/activity/discuss/quitWall?wid='+$scope.wid, function(rsp){
                $scope.users = null;
...
controller('msgCtrl',['$scope','http2',function($scope,http2) {
    ...
    $scope.doSearch = function(page) {
        if (!page)
            ...
            $scope.page.at = page;
        var url = '/rest/mp/activity/discuss/messages';
        url += '?wid='+$scope.wid;
    ...
    $scope.resetWall = function() {
        ...
        if (vcode === $scope.wall.title) {
            http2.get('/rest/mp/activity/discuss/resetWall?wid='+$scope.wid, function(rsp){
                $scope.messages = [];
                $scope.page.total = 0;
                $scope.page.at = 1;
            });
