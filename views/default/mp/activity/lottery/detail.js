xxtApp.controller('RouletteCtrl',['$scope','http2','$location',function($scope,http2,$location) {
    $scope.awardTypes = [
        {n:'未中奖',v:'0'},
        {n:'应用积分',v:'1'},
        {n:'奖励重玩',v:'2'},
    ...
    http2.get('/rest/mp/activity/lottery?lid='+$scope.lid, function(rsp){
...
.controller('SettingCtrl',['$scope','http2',function($scope,http2) {
    ...
    $scope.updateTime = function(name) {
        ...
        var p = {};
        p[name] = time/1000;
        http2.post('/rest/mp/activity/lottery/update?lid='+$scope.lid, p);
    };
    ...
    $scope.setPic = function(){
        $scope.$broadcast('picgallery.open', function(url){
            ...
            http2.post('/rest/mp/activity/lottery/update?lid='+$scope.lid, nv, function() {
...
.controller('AwardCtrl',['$scope','http2','$rootScope',function($scope,http2,$rootScope) {
    ...
    $scope.addAward = function() {
        http2.get('/rest/mp/activity/lottery/addAward?lid='+$scope.lottery.lid+'&mpid='+$scope.lottery.mpid, function(rsp){
            $scope.lottery.awards.push(rsp.data);
        });
    };
    $scope.removeAward = function(award) {
        http2.get('/rest/mp/activity/lottery/delAward?aid='+award.aid, function(rsp){
    ...
    $scope.update = function(award,name) {
        var p = {};
        p[name] = award[name];
        http2.post('/rest/mp/activity/lottery/setAward?aid='+award.aid, p);
    };
}])
.controller('PlateCtrl',['$scope','http2',function($scope,http2) {
    http2.get('/rest/mp/activity/lottery/plate?lid='+$scope.lid, function(rsp){
        $scope.plate = rsp.data;
    });
    $scope.update = function(slot) {
        var p = {};
        p[slot] = $scope.plate[slot];
        http2.post('/rest/mp/activity/lottery/setPlate?lid='+$scope.lid, p);
    };
}])
.controller('resultCtrl',['$rootScope','$scope','http2',function($rootScope,$scope,http2) {
    var doSearch = function(page) {
        ...
        var url = '/rest/mp/activity/lottery/result';
    ...
    var doStat = function(){
        http2.get('/rest/mp/activity/lottery/stat?lid='+$scope.lid, function(rsp){
            $scope.stat = rsp.data;
        });
    };
    $scope.byAward = '';
    ...
    $scope.removeRoll = function(r) {
        ...
        if (vcode === $scope.lottery.title) {
            var url = '/rest/mp/activity/lottery/removeRoll?lid='+$scope.lid;
            if (r.openid && r.openid.length > 0)
                url += '&openid='+r.openid+'&src='+r.src;
            else
                url += '&mid='+r.mid;
            http2.get(url, function(rsp){
                $scope.refresh();
    ...
    $scope.clean = function() {
        ...
        if (vcode === $scope.lottery.title) {
            http2.get('/rest/mp/activity/lottery/clean?lid='+$scope.lid, function(rsp){
                $scope.refresh();
            });
        }
    };
    $scope.addChance = function() {
        ...
        if (vcode === $scope.lottery.title) {
            http2.get('/rest/mp/activity/lottery/addChance?lid='+$scope.lid, function(rsp){
                $rootScope.infomsg = rsp.data;
            });
        }
    };
    http2.get('/rest/mp/activity/enroll?page=1&size=9999', function(rsp){
