xxtApp.controller('ImportAddressbookModalInstCtrl',['$rootScope','$scope','$modalInstance','abid',function($rootScope,$scope,$modalInstance,abid){
    ...
    $scope.ok = function () {
        $('#formImport').ajaxSubmit({
            url: '/rest/mp/matter/addressbook/import?abid='+abid+'&cleanExistent='+$scope.cleanExistent,
            type:'POST',
...
.controller('abCtrl',['$scope','http2',function($scope,http2){
    $scope.back = function() {
    ...
    $scope.update = function(name) {
        ...
        http2.post('/rest/mp/matter/addressbook/update?abid='+$scope.editing.id, nv);
    };
    $scope.setPic = function(){
        $scope.$broadcast('picgallery.open', function(url){
            var t=(new Date()).getTime(),url=url+'?_='+t,nv={'pic':url};
            http2.post('/rest/mp/matter/addressbook/update?abid='+$scope.editing.id, nv, function(){
    ...
    $scope.$watch('abid', function(nv){
        http2.get('/rest/mp/matter/addressbook?abid='+nv, function(rsp){
...
.controller('rollCtrl',['$scope','$modal','http2',function($scope,$modal,http2){
    $scope.abbr = '';
    $scope.page = {at:1,size:30};
    $scope.doSearch = function() {
        var url = '/rest/mp/matter/addressbook/person?abid='+$scope.abid+'&page='+$scope.page.at+'&size='+$scope.page.size+'&abbr='+$scope.abbr;
    ...
    $scope.create = function(){
        http2.get('/rest/mp/matter/addressbook/personCreate?abid='+$scope.abid, function(rsp) {
            location.href='/page/mp/matter/ab/person?abid='+$scope.abid+'&id='+rsp.data.id;
        });
    };
    $scope.edit = function(person) {
        location.href = '/page/mp/matter/ab/person?id='+person.id;
    };
    ...
    $scope.showImport = function() {
        $modal.open({
            ...
            resolve:{
                abid:function(){return $scope.editing.id;}
            }
...
.controller('deptCtrl',['$scope','http2','$modal',function($scope,http2,$modal){
    $scope.addChild = function(node) {
        var url = '/rest/mp/matter/addressbook/addDept?abid='+$scope.abid;
    ...
    $scope.removeChild = function(child) {
        ...
        http2.get('/rest/mp/matter/addressbook/delDept?abid='+$scope.abid+'&id='+child.data.id, function(rsp){
            walk($scope.tree);
        });
    };
    $scope.toggleChild = function(child) {
        if (!child.loaded) {
            child.loaded = true;
            http2.get('/rest/mp/matter/addressbook/dept?abid='+$scope.abid+'&pid='+child.data.id, function(rsp){
                var depts = rsp.data;
    ...
    $scope.updateDept = function(prop) {
        ...
        http2.post('/rest/mp/matter/addressbook/updateDept?abid='+$scope.abid+'&id='+$scope.editingDept.id, nv);
    };
    $scope.setDeptParent = function() {
        $modal.open({
            ...
            size:'lg',
            resolve:{
                abid:function(){return $scope.abid;},
                onlyOne:function(){return true;}
            }
        }).result.then(function(selected){
            http2.get('/rest/mp/matter/addressbook/setDeptParent?id='+$scope.editingDept.id+'&pid='+selected.id, function(rsp){
    ...
    $scope.cleanDeptParent = function() {
        http2.get('/rest/mp/matter/addressbook/setDeptParent?id='+$scope.editingDept.id+'&pid=0', function(rsp){
    ...
    $scope.tree = {children:[]};
    $scope.$watch('abid', function(nv){
        http2.get('/rest/mp/matter/addressbook/dept?abid='+$scope.abid, function(rsp){
            var depts = rsp.data;
