  ...
  ngApp.provider.controller('ctrlSetting', [
    ...
    function ($scope, http2, mediagallery) {
      $scope.update = function (name) {
        ...
        http2
          .post('/rest/pl/be/sns/wx/update?site=' + $scope.wx.plid, p)
          .then(function (rsp) {
            if (name === 'token') {
              $scope.wx.joined = 'N'
            }
      ...
      $scope.checkJoin = function () {
        http2
          .get('/rest/pl/be/sns/wx/checkJoin?site=' + $scope.wx.plid)
          .then(function (rsp) {
            if (rsp.data === 'Y') {
              $scope.wx.joined = 'Y'
            }
      ...
      $scope.$watch('wx', function (wx) {
        ...
        $scope.url =
          ...
          '//' +
          location.host +
          '/rest/site/sns/wx/api?site=platform'
      })
      $scope.syncUsers = function () {
        $scope.$root.progmsg = '开始更新'
        $scope.backRunning = true
        let finish = 0
        let doRefresh = function (step, nextOpenid) {
          let url, params
          url = '/rest/pl/fe/site/user/fans/refreshAll?site=platform'
          params = []
      ...
      $scope.syncGroups = function () {
        $scope.backRunning = true
        http2
          .get('/rest/pl/fe/site/user/fans/refreshGroup?site=platform')
          .then(function (rsp) {
            $scope.backRunning = false
