'use strict';
var ngApp = angular.module('app', ['ngRoute', 'ui.bootstrap', 'ui.tms', 'http.ui.xxt', 'notice.ui.xxt', 'protect.ui.xxt']);
...
ngApp.controller('ctrlAccount', ['$scope', '$uibModal', 'http2', 'noticebox', function ($scope, $uibModal, http2, noticebox) {
    function doSearch(pageAt) {
        ...
        http2.post('/rest/pl/be/user/account/list', _oFilter, {
            page: _oPage
        }).then(function (rsp) {
    ...
    $scope.remove = function (user) {
        var vcode;
        vcode = prompt('是否要删除用户账号？若是，请输入用户昵称。');
    ...
    $scope.forbid = function (user) {
        var vcode;
        vcode = prompt('是否要停用用户账号？若是，请输入用户昵称。');
        if (vcode === user.nickname) {
            http2.get('/rest/pl/be/user/account/forbid?uid=' + user.uid).then(function () {
                user.forbidden = 1
            });
        }
    };
    $scope.active = function (user) {
        var vcode;
        vcode = prompt('是否要激活用户账号？若是，请输入用户昵称。');
        if (vcode === user.nickname) {
            http2.get('/rest/pl/be/user/account/active?uid=' + user.uid).then(function () {
                user.forbidden = 0
            });
        }
    };
    ...
    $scope.forbideUser = function (user) {
        if (window.confirm('确定关闭帐号【' + user.uname + '】？')) {
            http2.post('/rest/pl/be/user/account/forbide', {
                uid: user.uid
            }).then(function (rsp) {
                user.forbidden = '1';
            });
    ...
    $scope.activeUser = function (user) {
        if (window.confirm('确定关闭帐号【' + user.uname + '】？')) {
            http2.post('/rest/pl/be/user/account/active', {
                uid: user.uid
            }).then(function (rsp) {
                user.forbidden = '0';
            });
