define(['frame'], function (ngApp) {
  ngApp.provider.controller('ctrlInvite', [
    ...
    'srvInvite',
    'http2',
    function ($scope, $q, $uibModal, srvInvite, http2) {
      ...
      $scope.closeInvite = function () {
        http2
          .get('/rest/pl/fe/invite/close?invite=' + _oInvite.id)
          .then(function (rsp) {
            _oInvite.state = '0'
          })
      }
      $scope.openInvite = function () {
        http2
          .get('/rest/pl/fe/invite/open?invite=' + _oInvite.id)
          .then(function (rsp) {
            _oInvite.state = '1'
          })
      }
        ...
        $uibModal
          .open({
            ...
            controller: [
              ...
              function ($mi, $scope) {
                $scope.code = {}
                ;['stop', 'expire_at', 'max_count', 'remark'].forEach(function (
                  prop
                ) {
                  if (prop == 'expire_at') {
                    if (oCode[prop] == '0') {
                ...
                $scope.ok = function () {
                  var regx = /^[0-9]\d*$/
                  if (
                    $scope.code.max_count === '' ||
                    !regx.test($scope.code.max_count)
                  ) {
                    alert('请输入正确的使用次数值')
                    return false
                  }
          ...
          .result.then(function (oNewCode) {
            http2
              .post('/rest/pl/fe/invite/code/update?code=' + oCode.id, oNewCode)
              .then(function (rsp) {
                ;['stop', 'expire_at', 'max_count', 'remark'].forEach(function (
                  prop
                ) {
      ...
      $scope.logList = function () {
        ...
        defer = $q.defer()
        url =
          '/rest/pl/fe/invite/log/list?invite=' + _oInvite.id + _oLogPage.join()
        http2.get(url).then(function (rsp) {
      ...
      $scope.relayList = function () {
        ...
        defer = $q.defer()
        url =
          '/rest/pl/fe/invite/relayList?matter=' +
          _oInvite.matter_type +
          ',' +
          _oInvite.matter_id +
          _oRelayPage.join()
        http2.get(url).then(function (rsp) {
