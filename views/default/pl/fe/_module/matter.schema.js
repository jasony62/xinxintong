        ...
        return {
          setOptGroup: function (oApp, oSchema) {
            ...
            if (!oSchema || !/single|multiple/.test(oSchema.type)) {
              ...
              $uibModal
                .open({
                  templateUrl:
                    '/views/default/pl/fe/matter/enroll/component/setOptGroup.html?_=1',
                  controller: [
                    '$scope',
                    '$uibModalInstance',
                    function ($scope2, $mi) {
                      ...
                      oApp.dataSchemas.forEach(function (oAppSchema) {
                        if (
                          oAppSchema.type === 'single' &&
                          oAppSchema.id !== oSchema.id
                        ) {
                      ...
                      $scope2.toggleGroup = function (oGroup) {
                        ...
                        if (oGroup.assocOp && oGroup.assocOp.schemaId) {
                          for (
                            var i = 0, ii = singleSchemas.length;
                            i < ii;
                            i++
                          ) {
          ...
          setVisibility: function (oApp, oSchema) {
            return $uibModal.open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/schema/setVisibility.html?_=3',
              controller: [
                '$scope',
                '$uibModalInstance',
                function ($scope2, $mi) {
                  ...
                  oApp.dataSchemas.forEach(function (oAppSchema) {
                    if (
                      /single|multiple/.test(oAppSchema.type) &&
                      oAppSchema.id !== oSchema.id
                    ) {
                      ...
                      if (_oBeforeRules && _oBeforeRules[oAppSchema.id]) {
                        var oBeforeRule
                        for (
                          var i = 0, ii = oAppSchema.ops.length;
                          i < ii;
                          i++
                        ) {
                          if (
                            _oBeforeRules[oAppSchema.id].indexOf(
                              oAppSchema.ops[i].v
                            ) !== -1
                          ) {
                            oBeforeRule = {
