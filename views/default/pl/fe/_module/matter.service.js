                            ...
                            return {
                                ...
                                execute: function (members, schemas) {
                                    ...
                                    function schemaUser(schemas, rounds) {
                                        http2.post('/rest/pl/fe/site/member/schema/importSchema?site=' + oMatter.siteid + '&id=' + oMatter.id + '&rounds=' + rounds, {
                                            'schemas': schemas,
                                            'users': ids
                                        }).then(function (rsp) {
...
service('tkRoundCron', ['$rootScope', '$q', '$uibModal', 'http2', function ($rootScope, $q, $uibModal, http2) {
    ...
    this.example = function (oRule) {
        http2.post('/rest/pl/fe/matter/' + this.matter.type + '/round/getCron', {
            roundCron: oRule
        }).then(function (rsp) {
            oRule.case = rsp.data;
        });
...
service('srvTimerNotice', ['$rootScope', '$parse', '$q', '$timeout', 'http2', 'tkRoundCron', function ($rootScope, $parse, $q, $timeout, http2, tkRoundCron) {
    function fnDbToLocal(oDb, oLocal) {
        ...
        oLocal.task.task_arguments = oDb.task_arguments ? oDb.task_arguments : {
            page: ''
        };
...
service('tkGroupApp', ['$q', '$uibModal', 'http2', function ($q, $uibModal, http2) {
    this.choose = function (oMatter, bMultiple) {
        ...
        http2.post('/rest/script/time', {
            html: {
                'groupApp': '/views/default/pl/fe/matter/enroll/component/chooseGroupApp.html'
            }
        }).then(function (rsp) {
            return $uibModal.open({
...
factory('tkEntryRule', ['$rootScope', '$timeout', 'noticebox', 'http2', 'srvSite', 'tkEnrollApp', 'tkGroupApp', function ($rootScope, $timeout, noticebox, http2, srvSite, tkEnrollApp, tkGroupApp) {
    ...
    function TK(oMatter, oSns, bNoSave, aExcludeRules) {
        ...
        this.removeMschema = function (mschemaId) {
            if (!_oRule || !_oRule.member || !angular.isObject(_oRule.member) || Object.keys(_oRule.member).length === 0)
                return true;
            if (!mschemaId)
