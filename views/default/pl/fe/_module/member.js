...
xxtMembers.service('userSetAsParam', [
  function () {
        ...
        default:
          ...
          if (userSet.depts && userSet.depts.length) {
            for (i in userSet.depts) {
              ...
              var newUs = {
                identity:
                  dept.id + (tagIds.length > 0 ? ',' + tagIds.join(',') : ''),
...
xxtMembers.controller('MemberAclUserPickerController', [
  '$scope',
  '$uibModalInstance',
  'userSetAsParam',
  'params',
  function ($scope, $mi, userSetAsParam, params) {
...
xxtMembers.controller('MemberAclController', [
  '$rootScope',
  '$scope',
  'http2',
  '$timeout',
  '$uibModal',
  function ($rootScope, $scope, http2, $timeout, $uibModal) {
    var setObjMemberSchemas = function () {
      var aMemberSchemas, i
      $scope.objMemberSchemas = angular.copy($scope.memberschemas)
      aMemberSchemas = $scope.obj[$scope.propMemberSchemas]
        ? $scope.obj[$scope.propMemberSchemas].trim()
        : ''
      aMemberSchemas =
        aMemberSchemas.length === 0 ? [] : aMemberSchemas.split(',')
      for (i in $scope.objMemberSchemas) {
        $scope.objMemberSchemas[i].checked =
          aMemberSchemas.indexOf($scope.objMemberSchemas[i].id) !== -1
            ? 'Y'
            : 'N'
      }
    }
    $scope.setAccessControl = function () {
      ...
      if ($scope.memberschemas.length === 1) {
        $scope.obj[$scope.propMemberSchemas] =
          $scope.obj[$scope.propAccess] === 'Y'
            ? $scope.memberschemas[0].id
            : ''
        $scope.objMemberSchemas[0].checked =
          $scope.obj[$scope.propAccess] === 'Y' ? 'Y' : 'N'
    ...
    $scope.setMemberschema = function (api) {
      var eapis,
        p = {}
      eapis = $scope.obj[$scope.propMemberSchemas]
        ? $scope.obj[$scope.propMemberSchemas].trim()
        : ''
      eapis = eapis.length === 0 ? [] : eapis.split(',')
      api.checked === 'Y'
        ? eapis.push(api.id)
        : eapis.splice(eapis.indexOf(api.id), 1)
    ...
    $scope.openAclSelector = function () {
      $uibModal
        .open({
          templateUrl:
            '/views/default/pl/fe/_module/userpicker.html?_=' +
            new Date().getTime(),
          resolve: {
            params: function () {
              return {
    ...
    $scope.changeAcl = function (newAcl, state) {
      http2.post($scope.changeAclUrl, newAcl).then(function (rsp) {
        if (newAcl.id === undefined) newAcl.id = rsp.data.id
    ...
    $scope.removeAcl = function (acl, event) {
      ...
      if (acl.id === undefined) $scope.obj[$scope.propAcl].splice(i, 1)
      else {
    ...
    http2
      .get(
        '/rest/pl/fe/site/member/schema/list?site=' + $scope.siteId + '&valid=Y'
      )
      .then(function (rsp) {
...
xxtMembers.directive('memberacl', function () {
  return {
    ...
    templateUrl:
      '/views/default/pl/fe/_module/memberacl.html?_=' + new Date().getTime(),
  }
})
xxtMembers.controller('UserPickerController', [
  'http2',
  '$scope',
  function (http2, $scope) {
    var getPickedMemberSchema = function () {
      var i,
        id = $scope.userSet.userScope.split('_').pop()
      for (i in $scope.memberschemas) {
        if (id === $scope.memberschemas[i].id) return $scope.memberschemas[i]
      }
    }
    ...
    $scope.canGroup = function () {
      return (
        !$scope.userConfig || $scope.userConfig.userScope.indexOf('G') !== -1
      )
    }
    $scope.canMember = function () {
      return (
        !$scope.userConfig || $scope.userConfig.userScope.indexOf('M') !== -1
      )
    }
    $scope.pickMp = function (mp) {
      !$scope.userSet.childmps && ($scope.userSet.childmps = [])
      if (mp.checked === 'Y') $scope.userSet.childmps.push(mp)
      else
    ...
    $scope.pickGroup = function (g) {
      ...
      if (g.checked === 'Y') $scope.userSet.fansGroup.push(g)
      else
    ...
    $scope.$watch('userSet.userScope', function (nv) {
      if (nv && nv.length) {
        if (nv === 'mp') {
          ...
          http2
            .get(
              $scope.memberschema.url +
                '/memberSelector?site=' +
                $scope.siteId +
                '&id=' +
                $scope.memberschema.id
            )
            .then(function (rsp) {
    ...
    if ($scope.canMember()) {
      http2
        .get('/rest/pl/fe/site/member/schema/list?site=' + $scope.siteId)
        .then(function (rsp) {
          $scope.memberschemas = rsp.data
        })
    }
...
xxtMembers.directive('userpicker2', [
  'http2',
  function (http2) {
    return {
      restrict: 'EA',
