'use strict'
angular.module('sys.chart', []).provider('srvChart', function () {
  var _oConfig
      ...
      return {
              ...
              labels: {
                formatter: function () {
                  if (_oConfig && _oConfig.label === 'percentage') {
                    return (
                      new Number((this.value / item.sum) * 100).toFixed(0) + '%'
                    )
                  } else {
                    return this.value
                  }
              ...
              bar: {
                dataLabels: {
                  ...
                  formatter: function () {
                    if (_oConfig && _oConfig.label === 'percentage') {
                      return (
                        new Number((this.y / item.sum) * 100).toFixed(2) + '%'
                      )
                    } else {
                      return this.y
                    }
                  ...
                  formatter: function () {
                    if (_oConfig && _oConfig.label === 'percentage') {
                      return (
                        ...
                        this.point.name +
                        '</b>:' +
                        new Number(this.percentage).toFixed(2) +
                        '%'
                      )
                    } else {
        ...
        drawNumPie: function (item, schema) {
          ...
          item.records.forEach(function (record) {
            var recVal = record.data[schema.id]
              ? parseInt(record.data[schema.id])
              : 0
            sum += recVal
            series.push({
              name: recVal,
