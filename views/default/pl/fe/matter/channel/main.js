define(['frame'], function (ngApp) {
    ngApp.provider.controller('ctrlMain', ['$scope', 'http2', 'mediagallery', 'srvSite', 'srvTag', 'cstApp', function ($scope, http2, mediagallery, srvSite, srvTag, cstApp) {
        var _oEditing;
        ...
        $scope.update = function (name) {
            ...
            http2.post('/rest/pl/fe/matter/channel/update?id=' + $scope.id, modifiedData).then(function () {
                if (name === 'orderby') {
                    http2.get('/rest/pl/fe/matter/channel/get?id=' + $scope.id).then(function (rsp) {
                        _oEditing = rsp.data;
                    });
                }
            });
        ...
        $scope.remove = function () {
            if (window.confirm('确定删除？')) {
                http2.get('/rest/pl/fe/matter/channel/remove?id=' + $scope.id).then(function (rsp) {
                    location = '/rest/pl/fe';
                });
            }
        };
