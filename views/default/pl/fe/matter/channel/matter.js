define(['frame'], function (ngApp) {
    ngApp.provider.controller('ctrlMatter', ['$scope', 'http2', 'srvSite', 'cstApp', function ($scope, http2, srvSite, cstApp) {
        ...
        $scope.getTopMatters = function () {
            http2.get('/rest/pl/fe/matter/channel/mattersList?id=' + _oEditing.id + '&weight=top').then(function (rsp) {
                $scope.topMatters = rsp.data.matters;
            });
        };
        $scope.getMatters = function (pageAt) {
            pageAt && (_oPage.at = pageAt);
            http2.get('/rest/pl/fe/matter/channel/mattersList?id=' + _oEditing.id + '&weight=center' + '&page=' + _oPage.at + '&size=' + _oPage.size).then(function (rsp) {
                $scope.matters = rsp.data.matters;
                $scope.page.total = rsp.data.total;
            });
        };
        $scope.getBottomMatters = function () {
            http2.get('/rest/pl/fe/matter/channel/mattersList?id=' + _oEditing.id + '&weight=bottom').then(function (rsp) {
                $scope.bottomMatters = rsp.data.matters;
            });
        };
