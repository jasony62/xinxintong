define(['frame'], function (ngApp) {
  ngApp.provider.controller('ctrlMatter', [
    '$scope',
    'http2',
    'srvSite',
    'cstApp',
    function ($scope, http2, srvSite, cstApp) {
      ...
      $scope.createArticle = function () {
        http2
          .get('/rest/pl/fe/matter/article/create?site=' + $scope.siteId)
          .then(function (rsp) {
            ...
            http2
              .post(
                '/rest/pl/fe/matter/channel/addMatter?site=' +
                  $scope.siteId +
                  '&channel=' +
                  _oEditing.id,
                relations
              ...
              .then(function (rsp) {
                location.href =
                  '/rest/pl/fe/matter/article?site=' +
                  $scope.siteId +
                  '&id=' +
                  article.id
              })
      ...
      $scope.createLink = function () {
        http2
          .get('/rest/pl/fe/matter/link/create?site=' + $scope.siteId)
          .then(function (rsp) {
            var link = rsp.data,
              relations = {
                matter: [link],
        ...
        srvSite
          ...
          .then(function (result) {
            if (result.matters && result.matters.length) {
              ...
              http2
                .post(
                  '/rest/pl/fe/matter/channel/setfixed?site=' +
                    $scope.siteId +
                    '&id=' +
                    $scope.id +
                    '&pos=' +
      ...
      $scope.cancelFixed = function (matter, pos) {
        ...
        http2
          .post(
            '/rest/pl/fe/matter/channel/unfixed?site=' +
              $scope.siteId +
              '&id=' +
              $scope.id +
              '&pos=' +
      ...
      $scope.getTopMatters = function () {
        http2
          .get(
            '/rest/pl/fe/matter/channel/mattersList?id=' +
              _oEditing.id +
              '&weight=top'
          )
          .then(function (rsp) {
      ...
      $scope.$on('my-sorted', function (ev, val) {
        ...
        $scope.topMatters.splice(
          val.to,
          0,
          $scope.topMatters.splice(val.from, 1)[0]
        )
        ...
        http2
          .post(
            '/rest/pl/fe/matter/channel/sortMatters?id=' +
              _oEditing.id +
              '&weight=top',
            $scope.topMatters
          )
