define(['frame'], function (ngApp) {
  'use strict'
  ngApp.provider.controller('ctrlEditor', [
    '$scope',
    '$sce',
    '$q',
    'noticebox',
    ...
    function (
      ...
      function _afterGetApp(oApp) {
            ...
            if (oRecord.data[oSchema.id]) {
              ...
              if (oSchema.type == 'multitext') {
                ...
                if (
                  ...
                  oRecord.data[oSchema.id].length
                ) {
        ...
        oResumable = new Resumable({
          target:
            '/rest/site/fe/matter/enroll/record/uploadFile?site=' +
            oApp.siteid +
            '&app=' +
            oApp.id,
          testChunks: false,
      ...
      function doSave() {
        ...
        for (var k in oRecord.data) {
          if (
            k !== 'member' &&
            _oApp._schemasById[k] &&
            _oApp._schemasById[k].type == 'multitext'
          ) {
      ...
      $scope.agree = function (oRecord, oSchema, oAgreed, oItemId) {
        srvEnlRec
          .agree(oRecord.enroll_key, oSchema.id, oAgreed, oItemId)
          .then(function () {})
      }
