define(['schema', 'wrap', 'matterSchema'], function (schemaLib, wrapLib) {
  'use strict'
  ...
  ngMod.provider('srvEnrollSchema', function () {
    ...
    this.$get = [
      ...
      'srvEnrollPage',
      function ($uibModal, $q, http2, srvApp, srvAppPage) {
        var _self = {
          makePagelet: function (content) {
            ...
            $uibModal
              .open({
                templateUrl:
                  '/views/default/pl/fe/matter/enroll/component/pagelet.html?_=1',
                controller: [
                  '$scope',
                  '$uibModalInstance',
                  'mediagallery',
                  function ($scope2, $mi, mediagallery) {
                    var tinymceEditor
            ...
            srvApp.get().then(function (oApp) {
                ...
                if (oSchema.required === 'Y') {
                  if (oSchema.type === 'shorttext') {
                    ...
                    if (oSchema.title.indexOf('姓名') !== -1) {
                      if (
                        !oNicknameSchema ||
                        oSchema.title.length < oNicknameSchema.title.length
                      ) {
                        oNicknameSchema = oSchema
                      }
              ...
              srvApp.update(updatedAppProps).then(function (oUpdatedApp) {
                function fnUpdateOnePage(index) {
                  srvAppPage
                    .update(changedPages[index], ['dataSchemas', 'html'])
                    .then(function () {
                      index++
   ...
   * 所有题目
   */
  ngMod.controller('ctrlSchemaList', [
    '$scope',
    '$timeout',
    '$sce',
    '$uibModal',
    ...
    function (
        ...
        $uibModal
          .open({
            ...
            controller: [
              ...
              '$timeout',
              'noticebox',
              function ($mi, $scope2, $timeout, noticebox) {
                $timeout(function () {
                  var oResumable = new Resumable({
                    target:
                      '/rest/pl/fe/matter/enroll/attachment/upload?site=' +
                      oApp.siteid +
                      '&app=' +
                      oApp.id,
                    testChunks: false,
                  })
                  oResumable.assignBrowse(
                    document.getElementById('addAttachment')
                  )
                  ...
                  oResumable.on('progress', function (file, event) {
                    $scope.$apply(function () {
                      noticebox.progress(
                        '正在上传文件：' +
                          Math.floor(oResumable.progress() * 100) +
                          '%'
                      )
                    })
                  })
                  oResumable.on('complete', function () {
                    ...
                    http2
                      .post(
                        '/rest/pl/fe/matter/enroll/attachment/add?site=' +
                          oApp.siteid +
                          '&app=' +
                          oApp.id,
                        posted
          ...
          .result.then(function (oAttachment) {
            ...
            mediaUrl =
              '/rest/site/fe/matter/enroll/attachment/get?site=' +
              oApp.siteid +
              '&app=' +
              oApp.id +
              '&attachment=' +
            ...
            switch (mediaType) {
              case 'vedio':
                html =
                  '<div><video controls="controls" preload="none" style="width:100%;">'
                html +=
                  '<source src="' +
                  mediaUrl +
                  '" type="' +
              ...
              case 'audio':
                html =
                  '<div><audio controls="controls" preload="none" style="width:100%;">'
                html +=
                  '<source src="' +
                  mediaUrl +
                  '" type="' +
      ...
      $scope.newByOtherApp = function (oProtoSchema, oOtherApp, oAfterSchema) {
        ...
        oNewSchema = schemaLib.newSchema(
          oProtoSchema.type,
          $scope.app,
          oProtoSchema
        )
      ...
      $scope.unassocWithOtherApp = function (oSchema, bOnlyAssocState) {
        ...
        if (oSchema.fromApp) {
          ...
          if (oAssocApp && oAssocApp.dataSchemas) {
                ...
                if (oMschema._schemas) {
                  oMschema._schemas.forEach(function (oMsSchema) {
                    if (oApp._schemasById[oMsSchema.id] === undefined) {
                      ...
                      oApp._schemasById[oMsSchema.id].mschema_id === msid
                    ) {
                      oMsSchema.assocState = 'yes'
                    } else {
      ...
      $scope.importByOther = function () {
        ...
        http2
          .post('/rest/script/time', {
            html: {
              other:
                '/views/default/pl/fe/matter/enroll/component/schema/importByOther',
            },
          })
          .then(function (rsp) {
            $uibModal
              .open({
                templateUrl:
                  '/views/default/pl/fe/matter/enroll/component/schema/importByOther.html?' +
                  rsp.data.html.other.time,
                controller: [
                  '$scope',
                  '$uibModalInstance',
                  function ($scope2, $mi) {
                    ...
                    $scope2.selectApp = function () {
                      ...
                      if (
                        oResult.fromApp &&
                        angular.isString(oResult.fromApp.data_schemas) &&
                        oResult.fromApp.data_schemas
                      ) {
                    ...
                    $scope2.selectSchema = function (schema) {
                      if (schema._selected) {
                        ...
                        oResult.schemas.splice(
                          oResult.schemas.indexOf(schema),
                          1
                        )
                      }
                    }
                    ...
                    $scope2.doSearch = function () {
                      var url =
                        '/rest/pl/fe/matter/enroll/list?site=' + _oApp.siteid
                    ...
                    $scope2.$watch(
                      ...
                      function (oNew, oOld) {
                        ...
                        if (!oResult.schemas || oResult.schemas.length === 0)
                          $scope2.disabled = true
                        ...
                        switch (oResult.purpose) {
                          ...
                          case 'scoreByInput':
                            if (!oResult.target) {
                              ...
                              if (!oTarget.ops || oTarget.ops.length === 0)
                                $scope2.disabled = true
                              if (
                                !oTarget.range ||
                                !oTarget.range.from ||
                ...
                switch (oResult.make) {
                  case 'copy': // 复制题目
                    fnGenNewSchema = function (oProtoSchema) {
                      ...
                      oProtoSchema.mschema_id &&
                        (oNewSchema.mschema_id = oProtoSchema.mschema_id)
                      ...
                      switch (oResult.purpose) {
                        case 'optionByInput':
                          if (
                            oResult.target &&
                            oResult.target.type &&
                            /single|multiple/.test(oResult.target.type)
                          ) {
                            fnGenNewSchema = function (oProtoSchema) {
                              if (
                                !/shorttext|longtext/.test(oProtoSchema.type)
                              ) {
                                return null
                              }
                              ...
                              if ((oTarget = oResult.target)) {
                                if (oNewSchema.type === 'multiple') {
                                  if (
                                    oTarget.limitChoice &&
                                    oTarget.limitChoice === 'Y'
                                  ) {
                                    oNewSchema.limitChoice = 'Y'
                                    if (oTarget.range) {
                                      oNewSchema.range = []
                                      if (oTarget.range.from) {
                                        oNewSchema.range.push(
                                          parseInt(oTarget.range.from)
                                        )
                                        if (oTarget.range.to)
                                          oNewSchema.range.push(
                                            parseInt(oTarget.range.to)
                                          )
                                      }
                                    }
                                  }
                        ...
                        case 'scoreByInput':
                          if (oResult.target) {
                            ...
                            oScoreProto = {}
                            oScoreProto.range = [
                              oResult.target.range.from,
                              oResult.target.range.to,
                            ]
                            oScoreProto.ops = []
                            ...
                            if (oResult.target.requireScore)
                              oScoreProto.requireScore = 'Y'
                            fnGenNewSchema = function (oProtoSchema) {
                              if (
                                !/shorttext|longtext/.test(oProtoSchema.type)
                              ) {
                        ...
                        oResult.schemas.forEach(function (oProtoSchema) {
                          ...
                          if ((oNewSchema = fnGenNewSchema(oProtoSchema))) {
                            if (oResult.requireGroup) {
                              var oNewGroupSchema
                              oNewGroupSchema = schemaLib.newSchema(
                                'html',
                                _oApp,
                                {
                              ...
                              oNewGroupSchema.content =
                                '<div>' + oProtoSchema.title + '</div>'
                  ...
                  case 'schema': // 直接生成题目
                    if (oResult.schemas && oResult.schemas.length) {
                        ...
                        case 'scoreByInput':
                          if ((oTarget = oResult.target)) {
                            if (
                              ...
                              if (oTarget.requireScore)
                                oProto.requireScore = true
                            }
                          }
                          oConfig = {
                            schemas: targetSchemas,
                      ...
                      if (oConfig) {
                        url =
                          '/rest/pl/fe/matter/enroll/schema/' + oResult.purpose
                        url += '?app=' + _oApp.id
      ...
      $scope.setOptGroup = function (oSchema) {
        srvMatterSchema.setOptGroup($scope.app, oSchema).then(function () {
          $scope.updSchema(oSchema)
        })
      }
      $scope.setSchemaParent = function (oSchema) {
        $uibModal
          .open({
            templateUrl:
              '/views/default/pl/fe/matter/enroll/component/schema/setSchemaParent.html?_=1',
            controller: [
              '$scope',
              '$uibModalInstance',
              function ($scope2, $mi) {
      ...
      $scope.setVisibility = function (oSchema) {
        srvMatterSchema.setVisibility($scope.app, oSchema).then(function () {
          $scope.updSchema(oSchema)
        })
      }
      $scope.setDefaultValue = function (oSchema) {
        $uibModal.open({
          templateUrl:
            '/views/default/pl/fe/matter/enroll/component/setDefaultValue.html?_=1',
          controller: [
            '$scope',
            '$uibModalInstance',
            function ($scope2, $mi) {
              ...
              $scope2.ok = function () {
                ...
                switch ($scope2.schema.type) {
                  ...
                  case 'multiple':
                    if (
                      !angular.equals(_oData.defaultValue, oSchema.defaultValue)
                    ) {
                      bChanged = true
                      var validKeys
                      validKeys = []
      ...
      $scope.setHistoryAssoc = function (oSchema) {
        ...
        $uibModal
          .open({
            templateUrl:
              '/views/default/pl/fe/matter/enroll/component/setHistoryAssoc.html?_=1',
            controller: [
              '$scope',
              '$uibModalInstance',
              function ($scope2, $mi) {
                ...
                _oApp.dataSchemas.forEach(function (oOther) {
                  if (
                    oOther.id !== oSchema.id &&
                    'shorttext' === oOther.type &&
                    oOther.history === 'Y'
                  ) {
      ...
      $scope.setDataSource = function (oSchema) {
        ...
        http2
          .post('/rest/script/time', {
            html: {
              source:
                '/views/default/pl/fe/matter/enroll/component/schema/setDataSource',
            },
          })
          .then(function (rsp) {
            $uibModal
              .open({
                templateUrl:
                  '/views/default/pl/fe/matter/enroll/component/schema/setDataSource.html?' +
                  rsp.data.html.source.time,
                controller: [
                  '$scope',
                  '$uibModalInstance',
                  'tkEnrollApp',
                  function ($scope2, $mi, tkEnlApp) {
                    ...
                    FnValidSchemas = {
                      ...
                      input: function (oSchema2) {
                        return (
                          'shorttext' === oSchema2.type &&
                          oSchema2.format === 'number'
                        )
                      },
                      option: function (oSchema2) {
                        return (
                          /single|multiple/.test(oSchema2.type) &&
                          oSchema2.dsOps
                        )
                      },
                    }
                    ...
                    if (oSchema.ds) {
                      if (oSchema.ds.app) {
                        tkEnlApp
                          .get(oSchema.ds.app.id)
                          .then(function (oSourceApp) {
                    ...
                    $scope2.selectApp = function () {
                      if (
                        angular.isString(_oResult.fromApp.data_schemas) &&
                        _oResult.fromApp.data_schemas
                      ) {
                        _oResult.fromApp.dataSchemas = JSON.parse(
                          _oResult.fromApp.data_schemas
                        )
                      }
                      _oResult.reset()
      ...
      $scope.setOptionsSource = function (oSchema) {
        ...
        $uibModal
          .open({
            templateUrl:
              '/views/default/pl/fe/matter/enroll/component/schema/setOptionsSource.html?_=1',
            controller: [
              '$scope',
              '$uibModalInstance',
              function ($scope2, $mi) {
                ...
                $scope2.selectApp = function () {
                  ...
                  if (
                    ...
                    oResult.fromApp.dataSchemas = JSON.parse(
                      oResult.fromApp.data_schemas
                    )
                ...
                $scope2.ok = function () {
                  var fromApp
                  if (
                    (fromApp = oResult.fromApp) &&
                    oResult.selected !== undefined
                  ) {
                    $mi.close({
                      action: 'ok',
                      app: {
      ...
      $scope.setSchemaSource = function (oSchema) {
        ...
        http2
          .post('/rest/script/time', {
            html: {
              source:
                '/views/default/pl/fe/matter/enroll/component/schema/setSchemaSource',
            },
          })
          .then(function (rsp) {
            $uibModal
              .open({
                templateUrl:
                  '/views/default/pl/fe/matter/enroll/component/schema/setSchemaSource.html?_=' +
                  rsp.data.html.source.time,
                controller: [
                  '$scope',
                  '$uibModalInstance',
                  function ($scope2, $mi) {
                          ...
                          switch (oResult.mode) {
                            case 'fromData':
                              fnValidSchema = function (oSchema) {
                                if (
                                  /longtext|url|multitext/.test(oSchema.type)
                                ) {
                                  $scope2.dsSchemas.push(oSchema)
                                } else if (
                                  /shorttext/.test(oSchema.type) &&
                                  ...
                                  $scope2.filterSchemas.push(
                                    angular.copy(oSchema)
                                  )
                                }
                              }
                              break
                    ...
                    $scope2.removeFilter = function (oFilter) {
                      oResult.filters.splice(
                        oResult.filters.indexOf(oFilter),
                        1
                      )
                    }
                    $scope2.ok = function () {
      ...
      $scope._removeSchema = function (oRemovedSchema) {
        ...
        srvEnrollSchema.submitChange(changedPages).then(function () {
          ...
          if (oRemovedSchema.mschema_id) {
            if (
              (oAssocSchema = $scope.unassocWithMschema(oRemovedSchema, true))
            ) {
          ...
          if (oRemovedSchema.fromApp) {
            if (
              (oAssocSchema = $scope.unassocWithOtherApp(oRemovedSchema, true))
            ) {
              oAssocSchema.assocState = ''
            }
          }
            ...
            if (prop && prop === 'weight') {
              srvApp.opData().then(function (oData) {
                if (oData.total > 0 || (oData.length && oData[0].total > 0)) {
                  $uibModal.open({
                    templateUrl:
                      '/views/default/pl/fe/matter/enroll/component/renewScore.html',
                    controller: [
                      '$scope',
                      '$uibModalInstance',
                      function ($scope2, $mi) {
      ...
      $scope.removeSchema = function (oRemovedSchema) {
        if (
          window.confirm(
            '确定从所有页面上删除登记项［' + oRemovedSchema.title + '］？'
          )
        ) {
      ...
      $scope.chooseSchema = function (event, oSchema) {
        ...
        if ($scope.app.scenario && oSchema.type === 'multiple') {
          angular.isString(oSchema.answer) &&
            (oSchema.answer = oSchema.answer.split(','))
      ...
      $scope.schemaHtml = function (schema) {
        if (schema) {
          ...
          return (
            '/views/default/pl/fe/matter/enroll/schema/' +
            schema.type +
            '.html?_=' +
            bust
          )
      ...
      http2
        .post('/rest/script/time', _IncludeSchemaTemplates)
        .then(function (rsp) {
      ...
      $scope.schemaEditorHtml = function () {
        if ($scope.activeOption) {
          return (
            '/views/default/pl/fe/matter/enroll/schema/option.html?_=' +
            (_IncludeSchemaTemplates.html.option.time || 1)
          )
        } else if ($scope.activeSchema) {
          return (
            '/views/default/pl/fe/matter/enroll/schema/main.html?_=' +
            (_IncludeSchemaTemplates.html.main.time || 1)
          )
        } else {
          return ''
        }
      ...
      $scope.refreshSchema = function (oSchema) {
        ...
        if (oSchema.id === '_round_id' && oApp.groupApp) {
          http2
            .get('/rest/pl/fe/matter/group/team/list?app=' + oApp.groupApp.id)
            .then(function (rsp) {
              var newOp, opById
              if (rsp.data.length) {
                opById = {}
      ...
      $scope.recycleSchema = function (oSchema) {
        $scope._appendSchema(oSchema).then(function () {
          $scope.app.recycleSchemas.splice(
            $scope.app.recycleSchemas.indexOf(oSchema),
            1
          )
   ...
   * 单个题目
   */
  ngMod.controller('ctrlSchemaEdit', [
    '$scope',
    function ($scope) {
      var _oEditing
      ...
      $scope.changeSchemaType = function () {
        ...
        if (
          false === schemaLib.changeType($scope.activeSchema, _oEditing.type)
        ) {
      ...
      $scope.$watch('activeSchema', function () {
          ...
          switch (_oEditing.type) {
            case 'multiple':
              ...
              if (!oActiveSchema.range) {
                oActiveSchema.range = [
                  1,
                  oActiveSchema.ops ? oActiveSchema.ops.length : 1,
                ]
              }
              break
          }
        ...
        if (window.sessionStorage) {
          ...
          if (window.sessionStorage.getItem('xxt.pl.protect.system')) {
            oSessionCached = window.sessionStorage.getItem(
              'xxt.pl.protect.system'
            )
