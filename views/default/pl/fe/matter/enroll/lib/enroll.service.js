define(['frame/templates', 'page'], function (FrameTemplates, pageLib) {
  'use strict'
  var BaseSrvEnrollRecord = function (
    $q,
    http2,
    noticebox,
    $uibModal,
    ...
    this._bSearch = function (url) {
      ...
      http2.post(url, that._oCriteria).then(function (rsp) {
        ...
        if (rsp.data) {
          ...
          if (rsp.data.total !== undefined) that._oPage.setTotal(rsp.data.total)
        } else {
          records = []
        }
    ...
    this._bGetAfter = function (oEnrollApp, fnCallback) {
      if (
        !oEnrollApp.tags ||
        !angular.isString(oEnrollApp.tags) ||
        oEnrollApp.tags.length === 0
      ) {
        oEnrollApp.tags = []
      } else {
      ...
      http2
        ...
        .then(function (rsp) {
          $uibModal
            .open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/recordFilter.html?_' +
  ...
  var ngModule = angular.module('service.enroll', [
    'ui.bootstrap',
    'ui.xxt',
    'service.matter',
  ])
  ngModule.provider('srvEnrollApp', function () {
      ...
      if (oApp.enrollApp && oApp.enrollApp.dataSchemas) {
        oApp.enrollApp.dataSchemas.forEach(function (oSchema) {
          if (oApp._schemasById[oSchema.id]) {
            oSchema.assocState =
              oApp._schemasById[oSchema.id].fromApp === oApp.enrollApp.id
                ? 'yes'
                : 'no'
          } else {
            oSchema.assocState = ''
    ...
    function _fnMapAssocGroupApp(oApp) {
      ...
      if (oApp.groupApp && oApp.groupApp.dataSchemas) {
        oApp.groupApp.dataSchemas.forEach(function (oSchema) {
          if (oApp._schemasById[oSchema.id]) {
            oSchema.assocState =
              oApp._schemasById[oSchema.id].fromApp === oApp.groupApp.id
                ? 'yes'
                : 'no'
          } else {
            oSchema.assocState = ''
    ...
    this.$get = [
      ...
      'noticebox',
      'srvSite',
      function ($q, $uibModal, http2, noticebox, srvSite) {
        ...
        _self = {
          ...
          update: function (names) {
            ...
            http2
              .post(_fnMakeApiUrl('update'), modifiedData)
              .then(function (rsp) {
   ...
   * round
   */
  ngModule.service('tkEnrollRound', [
    '$q',
    '$uibModal',
    'http2',
    'CstApp',
    function ($q, $uibModal, http2, CstApp) {
        ...
        this.controller = [
          ...
          function ($scope2, $mi) {
            ...
            $scope2.ok = function () {
              ...
              if (oNewRound.rid) {
                url =
                  '/rest/pl/fe/matter/enroll/round/update?app=' +
                  oApp.id +
                  '&rid=' +
                  oNewRound.rid
              } else {
            ...
            if (oRound.rid) {
              $scope2.downloadQrcode = function (url) {
                $(
                  '<a href="' + url + '" download="记录活动轮次二维码.png"></a>'
                )[0].click()
              }
              var rndEntryUrl
              ...
              $scope2.entry = {
                ...
                qrcode:
                  ...
                  oApp.siteid +
                  '&url=' +
                  encodeURIComponent(rndEntryUrl),
              }
              if (oApp.mission) {
                http2
                  .get(
                    '/rest/pl/fe/matter/mission/round/list?mission=' +
                      oApp.mission.id
                  )
                  .then(function (rsp) {
      ...
      this.edit = function (oApp, oRound) {
        ...
        defer = $q.defer()
        $uibModal
          .open(new RoundModal(oApp, oRound))
          .result.then(function (oNewRound) {
            defer.resolve(oNewRound)
          })
      ...
      this.remove = function (oApp, oRound) {
        var url =
          '/rest/pl/fe/matter/enroll/round/remove?app=' +
          oApp.id +
          '&rid=' +
          oRound.rid
  ...
  ngModule.provider('srvEnrollRound', function () {
    ...
    this.$get = [
      ...
      'srvEnrollApp',
      'tkEnrollRound',
      function ($q, $uibModal, http2, srvEnrollApp, tkEnlRnd) {
        return {
              ...
              http2
                ...
                .then(function (rsp) {
                  ...
                  rsp.data.rounds.forEach(function (rnd) {
                    rsp.data.active &&
                      (rnd._isActive = rnd.rid === rsp.data.active.rid)
                    _rounds.push(rnd)
                  })
    ...
    this.$get = [
      ...
      function (
        ...
        _ins.get = function (ek) {
          var defer = $q.defer()
          http2
            .get('/rest/pl/fe/matter/enroll/record/get?ek=' + ek)
            .then(function (rsp) {
              defer.resolve(rsp.data)
            })
          return defer.promise
        ...
        _ins.add = function (newRecord) {
          ...
          http2
            .post(
              '/rest/pl/fe/matter/enroll/record/add?site=' +
                _siteId +
                '&app=' +
                _appId,
              newRecord
        ...
        _ins.update = function (oRecord, oUpdated) {
          ...
          http2
            .post(
              '/rest/pl/fe/matter/enroll/record/update?site=' +
                _siteId +
                '&app=' +
                _appId +
                '&ek=' +
          ...
          $uibModal
            ...
            .result.then(function (result) {
              ...
              if (eks.length) {
                ...
                http2
                  .post(
                    '/rest/pl/fe/matter/enroll/record/batchTag?site=' +
                      _siteId +
                      '&app=' +
                      _appId,
                    posted
        ...
        _ins.remove = function (record) {
          if (window.confirm('确认删除？')) {
            http2
              .get(
                '/rest/pl/fe/matter/enroll/record/remove?app=' +
                  _appId +
                  '&ek=' +
                  record.enroll_key
              )
        ...
        _ins.restore = function (record) {
          if (window.confirm('确认恢复？')) {
            http2
              .get(
                '/rest/pl/fe/matter/enroll/record/recover?app=' +
                  _appId +
                  '&ek=' +
                  record.enroll_key
              )
        ...
        _ins.empty = function () {
          ...
          if (vcode === _ins._oApp.title) {
            http2
              .get('/rest/pl/fe/matter/enroll/record/empty?app=' + _appId)
              .then(function (rsp) {
        ...
        _ins.verifyAll = function () {
          if (
            window.confirm(
              '确定审核通过所有记录（共' + _ins._oPage.total + '条）？'
            )
          ) {
            http2
              .get(
                '/rest/pl/fe/matter/enroll/record/batchVerify?app=' +
                  _appId +
                  '&all=Y'
              )
              .then(function (rsp) {
        ...
        _ins.batchVerify = function (rows) {
          ...
          if (
            window.confirm(
              '确定审核通过选中的记录（共' +
                Object.keys(rows.selected).length +
                '条）？'
            )
          ) {
        ...
        _ins.export = function (joinDirs) {
          ...
          if (
            _ins._oCriteria.data &&
            Object.keys(_ins._oCriteria.data).length
          ) {
            var oFilterDat = {}
        ...
        _ins.exportImage = function (rid) {
          ...
          http2
            .post('/rest/script/time', {
              html: {
                export:
                  '/views/default/pl/fe/matter/enroll/component/record/exportBatch',
              },
            })
            .then(function (rsp) {
              $uibModal.open({
                templateUrl:
                  '/views/default/pl/fe/matter/enroll/component/record/exportBatch.html?_=' +
                  rsp.data.html.export.time,
                controller: [
                  '$scope',
                  '$uibModalInstance',
                  function ($scope2, $mi) {
                    http2
                      ...
                      .then(function (rsp) {
                        var BaseDownloadUrl =
                          '/rest/pl/fe/matter/enroll/export/image?site=' +
                          _siteId +
                          '&app=' +
                          _appId +
                          '&rid=' +
        ...
        _ins.importByOther = function () {
          ...
          $uibModal
            .open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/record/importByOther.html?_=1',
              controller: [
                '$scope',
                '$uibModalInstance',
                function ($scope2, $mi) {
                  function doSearchRnd(appId, oDataset) {
                    http2
                      .get(
                        '/rest/pl/fe/matter/enroll/round/list?app=' + appId,
                        {
                          page: oDataset.page,
                        }
                      )
                  ...
                  $scope2.$watch('data.fromApp', function (oFromApp) {
                    if (oFromApp) {
                      ...
                      http2
                        .get(
                          '/rest/pl/fe/matter/enroll/schema/compatible?app1=' +
                            _appId +
                            '&app2=' +
                            oFromApp.id
                        )
                    ...
                    function (oNewData) {
                      if (oNewData) {
                        $scope2.executable =
                          ...
                          !!oNewData.fromApp &&
                          !!oNewData.fromRnd &&
                          !!oNewData.compatibleSchemas &&
                          oNewData.compatibleSchemas.length > 0 &&
                          oNewData.countOfRecord > 0
                      }
                    },
                    true
                  )
            ...
            .result.then(function (_oData) {
              var url =
                '/rest/pl/fe/matter/enroll/record/importByOther?site=' +
                _siteId +
                '&app=' +
                _appId
          ...
          if (!eks || eks.length === 0) {
            ...
            http2
              .post('/rest/script/time', {
                html: {
                  export:
                    '/views/default/pl/fe/matter/enroll/component/record/exportToOther',
                },
              })
              .then(function (rsp) {
                $uibModal
                  .open({
                    templateUrl:
                      '/views/default/pl/fe/matter/enroll/component/record/exportToOther.html?_=' +
                      rsp.data.html.export.time,
                    controller: [
                      '$scope',
                      '$uibModalInstance',
                      function ($scope2, $mi) {
                        ...
                        $scope2.doSearch = function () {
                          var url =
                            '/rest/pl/fe/matter/enroll/list?site=' + _siteId
                          http2
                            .post(
                              url,
                              {
        ...
        _ins.transferVotes = function (oApp) {
          ...
          $uibModal
            .open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/transferVotes.html?_=1',
              controller: [
                '$scope',
                '$uibModalInstance',
                function ($scope2, $mi) {
                  ...
                  $scope2.selectSchema = function (oSchema) {
                    if (oSchema._selected) {
                      ...
                      oResult.votingSchemas.splice(
                        oResult.votingSchemas.indexOf(oSchema.id),
                        1
                      )
                    }
                  }
                  ...
                  $scope2.doSearch = function () {
                    var url =
                      '/rest/pl/fe/matter/enroll/list?site=' + oApp.siteid
                    http2
                      .post(
                        url,
                        {
        ...
        _ins.transferSchemaAndVotes = function (oApp) {
          ...
          $uibModal
            .open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/transferSchemaAndVotes.html?_=1',
              controller: [
                '$scope',
                '$uibModalInstance',
                function ($scope2, $mi) {
                  ...
                  $scope2.$watch(
                    ...
                    function () {
                      ...
                      if (
                        ...
                        oResult.votingSchemas.length === 0
                      )
            ...
            .result.then(function (oResult) {
              ...
              if (
                oResult.fromApp &&
                oResult.questionSchema &&
                oResult.answerSchema &&
                oResult.votingSchemas.length
              ) {
        ...
        _ins.transferGroupAndMarks = function (oApp) {
          ...
          $uibModal
            .open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/transferGroupAndMarks.html?_=1',
              controller: [
                '$scope',
                '$uibModalInstance',
                function ($scope2, $mi) {
        ...
        _ins.fillByOther = function (oApp) {
          ...
          $uibModal
            .open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/fillByOther.html?_=1',
              controller: [
                '$scope',
                '$uibModalInstance',
                function ($scope2, $mi) {
                  ...
                  $scope2.doSearch = function () {
                    ...
                    switch (oResult.matterType) {
                      case 'mschema':
                        url =
                          '/rest/pl/fe/site/member/schema/list?valid=Y&site=' +
                          oApp.siteid +
                          '&matter=' +
                          oApp.id +
                          ',enroll'
      ...
      function ($q, http2, $uibModal) {
        return {
          list: function (page, type, criteria) {
            ...
            url =
              '/rest/pl/fe/matter/enroll/log/list?logType=' +
              type +
              '&app=' +
              _appId +
              page._j()
          ...
          filter: function (type, criteria) {
            ...
            $uibModal.open({
              templateUrl:
                '/views/default/pl/fe/matter/enroll/component/logFilter.html?_=1',
              controller: [
                '$scope',
                '$uibModalInstance',
                'http2',
                function ($scope2, $mi, http2) {
                  ...
                  $scope2.doSearchRound = function () {
                    var url =
                      '/rest/pl/fe/matter/enroll/round/list?site=' +
                      _siteId +
                      '&app=' +
                      _appId +
                      $scope2.pageOfRound.j()
                    http2.get(url).then(function (rsp) {
   ...
   * filter
   */
  ngModule.controller('ctrlEnrollFilter', [
    '$scope',
    '$uibModalInstance',
    'dataSchemas',
    'criteria',
    'srvEnlRnd',
    'app',
    function ($scope, $mi, dataSchemas, lastCriteria, srvEnlRnd, oApp) {
      var canFilteredSchemas = []

      if (
        oApp.entryRule &&
        oApp.entryRule.scope &&
        oApp.entryRule.scope.group === 'Y' &&
        oApp.entryRule.group &&
        oApp.entryRule.group.id
      ) {
