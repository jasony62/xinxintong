define([], function () {
  ...
  return {
    ...
    newSchema: function (type, oApp, oProto) {
      ...
      if (prefab[type]) {
        ...
        oSchema.title =
          oProto && oProto.title
            ? oProto.title
            : prefab[type].title + ++countOfType
      } else {
        oSchema.title =
          oProto && oProto.title
            ? oProto.title
            : '填写项' + (oApp.dataSchemas.length + 1)
      ...
      if (oProto) {
        if (oProto.format !== undefined) oSchema.format = oProto.format
    ...
    changeType: function (schema, newType) {
      if (
        /single|multiple|score/.test(schema.type) &&
        !/single|multiple|score/.test(newType)
      ) {
        delete schema.ops
      }
      if (
        (schema.type === 'score' && newType !== 'score') ||
        (schema.type === 'multiple' && newType !== 'multiple')
      ) {
        delete schema.range
      }
