define(['frame'], function (ngApp) {
  ...
  ngApp.provider.controller('ctrlMain', [
    ...
    'srvEnrollApp',
    'srvTag',
    function ($scope, $anchorScroll, http2, noticebox, srvEnrollApp, srvTag) {
        ...
        if (window.confirm('确定删除[' + $scope.app.title + ']？')) {
          srvEnrollApp.remove().then(function () {
            if ($scope.app.mission) {
              ...
              location =
                '/rest/pl/fe?view=main&scope=activity&type=enroll&sid=' +
                $scope.app.siteid
            }
          })
        }
      ...
      $scope.exportAsTemplate = function () {
        var url
        url =
          '/rest/pl/fe/matter/enroll/export/appTemplate?site=' +
          $scope.app.siteid +
          '&app=' +
          $scope.app.id
        window.open(url)
  ...
  ngApp.provider.controller('ctrlAccess', [
    ...
    'srvEnrollApp',
    'tkEntryRule',
    function ($scope, srvEnlApp, tkEntryRule) {
      var _oApp, _oRule
