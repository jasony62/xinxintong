  ...
  ngApp.provider.controller('ctrlPage', [
    ...
    function ($scope, $location, srvEnrollApp, srvAppPage) {
      ...
      $scope.cleanPage = function () {
        if (
          window.confirm('确定清除页面【' + $scope.ep.title + '】的所有内容？')
        ) {
      ...
      $scope.delPage = function () {
        ...
        if (bUserd) {
          ...
          window.confirm('确定删除页面【' + $scope.ep.title + '】？')
        ) {
      ...
      $scope.$on(
        'xxt.matter.enroll.app.dataSchemas.modified',
      ...
      srvEnrollApp.get().then(function (app) {
        ...
        if (
          _oApp.entryRule &&
          _oApp.entryRule.scope.group === 'Y' &&
          _oApp.entryRule.group &&
          _oApp.entryRule.group.id
        ) {
