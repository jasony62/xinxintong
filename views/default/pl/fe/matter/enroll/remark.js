                    ...
                    if (oAssocRecords[oRemark.enroll_key]) {
                        ...
                        if (oRemark.data_id > 0) {
                            if (_oApp._schemasById[oRemark.schema_id]) {
                                ...
                                targetContent = tmsSchema.strRecData(oAssocRecords[oRemark.enroll_key]._data, [oTargetSchema], {
                                    fnDataFilter: function (dataId) {
                                        return dataId == oRemark.data_id;
                                    }
                                });
                            ...
                            /* 对记录进行评论 */
                            targetContent = tmsSchema.strRecData(oAssocRecords[oRemark.enroll_key]._data, _oApp.dynaDataSchemas, {
                                fnSchemaFilter: function (oSchema) {
                                    return oSchema.shareable === 'Y';
                                }
                            });
                            oRemarkTarget.content = targetContent;
                        }
        ...
        $scope.filter = function () {
            $uibModal.open({
                ...
                controller: ['$scope', '$uibModalInstance', function ($scope2, $mi) {
                    http2.post('/rest/pl/fe/matter/enroll/user/enrollee?site=' + $location.search().site + '&app=' + $location.search().id, {}).then(function (rsp) {
