define(['frame'], function (ngApp) {
    ...
    ngApp.provider.controller('ctrlCoinRule', ['$scope', 'http2', 'srvEnrollApp', function ($scope, http2, srvEnlApp) {
        ...
        _aDefaultRules = [{
            ...
            data: {
                act: 'site.matter.enroll.group.submit'
            },
            desc: '用户组A内每个用户都提交过记录',
        }, {
            data: {
            ...
            data: {
                act: 'site.matter.enroll.data.get.dislike'
            },
            desc: '用户A填写数据被反对',
        }, {
            data: {
            ...
            data: {
                act: 'site.matter.enroll.cowork.get.dislike'
            },
            desc: '用户A填写的协作数据被反对',
        }, {
            data: {
            ...
            data: {
                act: 'site.matter.enroll.remark.get.dislike'
            },
            desc: '用户A发表的留言被反对',
        }, {
            data: {
    ...
    ngApp.provider.controller('ctrlExportRule', ['$scope', 'noticebox', 'http2', 'srvEnrollApp', 'tkEnrollApp', function ($scope, noticebox, http2, srvEnlApp, tkEnlApp) {
        ...
        $scope.delConfig = function (oConfig) {
            noticebox.confirm('删除填写记录转发设置，确定？').then(function () {
                if (oConfig.id) {
                    http2.post('/rest/pl/fe/matter/enroll/updateTransmitConfig?app=' + $scope.app.id, {
                        method: 'delete',
                        data: oConfig
                    }).then(function () {
