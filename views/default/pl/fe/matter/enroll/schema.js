define(['frame'], function (ngApp) {
   ...
   * 导出模板，导入记录
   */
  ngApp.provider.controller('ctrlImport', [
    '$scope',
    '$uibModal',
    'http2',
    'noticebox',
    function ($scope, $uibModal, http2, noticebox) {
      $scope.importByExcel = function () {
        $uibModal.open({
          templateUrl:
            '/views/default/pl/fe/matter/enroll/component/importByExcel.html?_=2',
          controller: [
            '$scope',
            '$timeout',
            '$uibModalInstance',
            'srvEnrollRound',
            function ($scope2, $timeout, $mi, srvEnlRnd) {
              var oApp, oResu, oOptions
              oApp = $scope.app
              oResu = new Resumable({
                target:
                  '/rest/pl/fe/matter/enroll/import/upload?site=' +
                  oApp.siteid +
                  '&app=' +
                  oApp.id,
                testChunks: false,
              ...
              oResu.on('complete', function () {
                ...
                http2
                  .post(
                    '/rest/pl/fe/matter/enroll/import/endUpload?site=' +
                      oApp.siteid +
                      '&app=' +
                      oApp.id,
                    oPosted
              ...
              oApp.dataSchemas.forEach(function (oSchema) {
                if (
                  ...
                  (!oSchema.format || oSchema.format !== 'number')
                ) {
              ...
              if (
                ...
                oApp.entryRule.scope.member === 'Y' &&
                oApp.entryRule.member &&
                Object.keys(oApp.entryRule.member).length
              ) {
                oOptions.assoc.source = 'app.mschema'
              ...
              $scope2.$watch(
                ...
                function (nv) {
                  ...
                  if (oOptions.assoc.source) {
                    if (
                      !oOptions.assoc.intersected ||
                      Object.keys(oOptions.assoc.intersected).length === 0
                    ) {
                      $scope2.hasError = true
                    } else {
                      angular.forEach(
                      ...
                      if ($scope2.intersectedValidNum === 0)
                        $scope2.hasError = true
                    }
                  }
                },
      ...
      $scope.downloadTemplate = function () {
        var url =
          '/rest/pl/fe/matter/enroll/import/downloadTemplate?site=' +
          $scope.app.siteid +
          '&app=' +
          $scope.app.id
        window.open(url)
