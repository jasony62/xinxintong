...
ngApp = angular.module('app', ['ngRoute', 'ui.bootstrap', 'ui.tms', 'ui.xxt', 'http.ui.xxt', 'notice.ui.xxt', 'pl.const', 'service.matter', 'protect.ui.xxt']);
...
ngApp.controller('ctrlMain', ['$scope', 'http2', 'srvSite', function ($scope, http2, srvSite) {
    ...
    http2.post('/rest/script/time', {
        ...
        $scope.frameTemplates = {
            html: {
                'entryRule': '/views/default/pl/fe/_module/entryRule.html?_=' + rsp.data.html.entryRule.time
            }
        };
    });
...
ngApp.controller('ctrlSysTemplate', ['$scope', '$location', '$uibModal', 'http2', 'CstNaming', 'srvSite', 'tkEntryRule', function ($scope, $location, $uibModal, http2, CstNaming, srvSite, tkEntryRule) {
    ...
    $scope.doCreateUserScore = function () {
        var url;
        url = '/rest/pl/fe/matter/enroll/create/byMissionUser?mission=' + _missionId;
...
ngApp.controller('ctrlFile', ['$scope', 'http2', function ($scope, http2) {
    function _excelLoader() {
        if (Resumable) {
            ...
            r = new Resumable({
                target: '/rest/pl/fe/matter/enroll/create/uploadExcel?site=' + _siteId,
                testChunks: false,
            });
            r.assignBrowse(ele);
            ...
            r.on('complete', function () {
                ...
                http2.post('/rest/pl/fe/matter/enroll/create/byExcel?site=' + _siteId, posted).then(function (rsp) {
                    $mi.close({
                        source: 'file',
                        app: rsp.data
                    });
