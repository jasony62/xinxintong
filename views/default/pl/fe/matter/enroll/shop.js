...
ngApp.config([
  '$locationProvider',
  'srvSiteProvider',
  function ($locationProvider, srvSiteProvider) {
  ...
  function ($scope, http2, srvSite) {
    ...
    srvSite.get().then(function (oSite) {
      ...
      if (_missionId) {
        http2
          .get(
            '/rest/pl/fe/matter/mission/get?site=' +
              _siteId +
              '&id=' +
              _missionId
          )
...
ngApp.controller('ctrlSysTemplate', [
  '$scope',
  '$location',
  'http2',
  'srvSite',
  'tkEntryRule',
  function ($scope, $location, http2, srvSite, tkEntryRule) {
    ...
    $scope.doCreate = function () {
      ...
      http2.post(url, oConfig).then(function (rsp) {
        location.href =
          '/rest/pl/fe/matter/enroll/schema?site=' +
          _siteId +
          '&id=' +
          rsp.data.id
      })
    ...
    $scope.doCreateUserScore = function () {
      var url
      url =
        '/rest/pl/fe/matter/enroll/create/byMissionUser?mission=' + _missionId
    ...
    $scope.chooseTemplate = function () {
      ...
      http2.get(url).then(function (rsp) {
        ...
        oScenarioConfig.required =
          oScenarioConfig.can_repos !== 'D' ||
          oScenarioConfig.can_rank !== 'D' ||
          oScenarioConfig.can_repos !== 'D'
        ...
        if (oMission) {
          ...
          if (oMission.entryRule.scope) {
            if (oMission.entryRule.scope.member === 'Y') {
              ...
              srvSite
                .memberSchemaList(oMission)
                .then(function (aMemberSchemas) {
                  var oMschemasById = {}
                  aMemberSchemas.forEach(function (mschema) {
                    oMschemasById[mschema.id] = mschema
                  })
                  Object.keys(oMission.entryRule.member).forEach(function (
                    mschemaId
                  ) {
                    _oEntryRule.member[mschemaId] = {
                      title: oMschemasById[mschemaId].title,
                    }
                  })
                })
            }
  ...
  function ($scope, http2) {
    function _excelLoader() {
      if (Resumable) {
        ...
        r = new Resumable({
          target:
            '/rest/pl/fe/matter/enroll/create/uploadExcel?site=' + _siteId,
          testChunks: false,
        })
        r.assignBrowse(ele)
        ...
        r.on('complete', function () {
          ...
          http2
            .post(
              '/rest/pl/fe/matter/enroll/create/byExcel?site=' + _siteId,
              posted
            )
            .then(function (rsp) {
              $mi.close({
    ...
    $scope.$on('doCreate.shop', function () {
      if (oNewApp) {
        location.href =
          '/rest/pl/fe/matter/enroll?site=' + _siteId + '&id=' + choice.app.id
      }
    })
    window.chooseFile = function (file) {
      ...
      fReader.onload = function (evt) {
        ...
        url = '/rest/pl/fe/matter/enroll/create/byConfig?site=' + _siteId
        if (_missionId) {
          url += '&mission=' + _missionId
        }
