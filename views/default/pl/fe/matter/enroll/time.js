    ...
    ngApp.provider.controller('ctrlTime', ['$scope', '$uibModal', 'http2', 'noticebox', 'CstApp', 'srvEnrollApp', 'srvEnrollRound', function($scope, $uibModal, http2, noticebox, CstApp, srvEnlApp, srvEnlRnd) {
        function fnSetSyncMissionRound(bSetAppRound) {
            if (_oOptions.sync_mission_round === 'Y') {
                if (bSetAppRound) {
                    http2.get('/rest/pl/fe/matter/enroll/round/syncMissionRound?app=' + $scope.app.id, { autoBreak: false }).then(function(rsp) {
                    ...
                    http2.post('/rest/pl/fe/matter/enroll/update?app=' + $scope.app.id, { sync_mission_round: 'Y' }).then(function(rsp) {
                        $scope.app.sync_mission_round = 'Y';
                        $scope.doSearchRound();
                    });
                }
            } else {
                http2.post('/rest/pl/fe/matter/enroll/update?app=' + $scope.app.id, { sync_mission_round: 'N' }).then(function(rsp) {
        ...
        $scope.pageOfRound = {};
        $scope.options = _oOptions = { sync_mission_round: 'N' };
            ...
            if (_oOptions.sync_mission_round === 'Y' && _rounds.length === 1 && _rounds[0].start_at == 0) {
                $uibModal.open({
                    ...
                    controller: ['$scope', '$uibModalInstance', function($scope2, $mi) {
                        $scope2.result = { setAppRound: false };
                        $scope2.cancel = function() { $mi.dismiss(); };
                        $scope2.ok = function() { $mi.close($scope2.result); };
                    }]
