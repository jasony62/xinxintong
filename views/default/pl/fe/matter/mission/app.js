define(['frame'], function (ngApp) {
  'use strict'
  ngApp.provider.controller('ctrlApp', [
    '$scope',
    'http2',
    'facListFilter',
    'CstNaming',
    '$uibModal',
    function ($scope, http2, facListFilter, CstNaming, $uibModal) {
      ...
      $scope.addEnroll = function () {
        location.href =
          '/rest/pl/fe/matter/enroll/shop?site=' +
          _oMission.siteid +
          '&mission=' +
          _oMission.id
      }
      $scope.addSignin = function () {
        location.href =
          '/rest/pl/fe/matter/signin/plan?site=' +
          _oMission.siteid +
          '&mission=' +
          _oMission.id
      }
      $scope.addGroup = function () {
        location.href =
          '/rest/pl/fe/matter/group/plan?site=' +
          _oMission.siteid +
          '&mission=' +
          _oMission.id
      }
      $scope.addMemberschema = function () {
        var url, proto
        url =
          '/rest/pl/fe/site/member/schema/create?site=' + $scope.mission.siteid
        proto = {
          valid: 'Y',
          matter_id: _oMission.id,
          matter_type: _oMission.type,
          title: _oMission.title + '-通讯录',
        }
      ...
      $scope.copyMatter = function (evt, matter) {
        ...
        if (type == 'enroll') {
          $uibModal
            .open({
              templateUrl: '/views/default/pl/fe/_module/copyMatter.html?_=4',
              controller: [
                '$scope',
                '$uibModalInstance',
                'http2',
                function ($scope2, $mi, http2) {
                  ...
                  $scope2.$watch('criteria.isMatterData', function (nv) {
                    if (nv === 'Y') {
                      criteria.isMatterAction = 'Y'
                    }
                  })
                  $scope2.doMission = function () {
                    var url =
                        '/rest/pl/fe/matter/mission/list?site=' +
                        siteid +
                        '&fields=id,title',
                      params = {
                        byTitle: criteria.byTitle,
                      }
                    http2
                      .post(url, params, {
                        page: $scope2.pageOfMission,
                      })
                      .then(function (rsp) {
                        if (rsp.data) {
                  ...
                  $scope2.ok = function () {
                    $mi.close({
                      cpTimer: criteria.isMatterTimer,
                      cpCoinRule: criteria.isMatterCoinRule,
            ...
            .result.then(function (result) {
              ...
              http2.get(url).then(function (rsp) {
                location.href =
                  '/rest/pl/fe/matter/enroll/preview?site=' +
                  rsp.data.siteid +
                  '&id=' +
                  rsp.data.id
              })
      ...
      $scope.list = function () {
        ...
        url +=
          '&matterType=' +
          (_oCriteria.matterType === '' ? 'app' : _oCriteria.matterType)
