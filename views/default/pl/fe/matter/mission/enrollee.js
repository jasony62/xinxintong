define(['frame'], function (ngApp) {
  ...
  ngApp.provider.controller('ctrlEnrollee', [
    ...
    'facListFilter',
    'noticebox',
    function ($scope, $filter, http2, facListFilter, noticebox) {
      ...
      $scope.enrolleeList = function () {
        var url
        url =
          '/rest/pl/fe/matter/mission/user/enrolleeList?mission=' + _oMission.id
        http2.post(url, _oCriteria).then(function (rsp) {
          ...
          enrollees.forEach(function (oEnrollee) {
            ...
            ;[
              ...
              oEnrollee[prop] =
                oEnrollee[prop] > 0
                  ? $filter('date')(oEnrollee[prop] * 1000, dateFormat)
                  : ''
            })
            oEnrollee.total_elapse =
              oEnrollee.total_elapse > 0
                ? fnReadableElapse(oEnrollee.total_elapse)
                : ''
      ...
      $scope.createEnlAppByUser = function () {
        var url
        url =
          '/rest/pl/fe/matter/enroll/create/byMissionUser?mission=' +
          _oMission.id
      ...
      $scope.renewName = function () {
        var url
        url = '/rest/pl/fe/matter/mission/user/renewName'
        url += '?mission=' + _oMission.id
        http2.get(url).then(function (rsp) {
          $scope.enrolleeList()
          noticebox.success('更新完成')
