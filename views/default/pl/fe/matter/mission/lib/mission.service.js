            ...
            return {
                ...
                add: function () {
                    http2.post('/rest/script/time', {
                        html: {
                            'editor': '/views/default/pl/fe/matter/enroll/component/roundEditor'
                        }
                    }).then(function (rsp) {
                        $uibModal.open({
                            ...
                            controller: ['$scope', '$uibModalInstance', function ($scope, $mi) {
                                ...
                                $scope.ok = function () {
                                    http2.post(_RestURL + 'add?site=' + _siteId + '&mission=' + _missionId, oNewRound).then(function (rsp) {
                        ...
                        $uibModal.open({
                            ...
                            controller: ['$scope', '$uibModalInstance', function ($scope, $mi) {
                                $scope.round = {
                                    rid: oRound.rid,
                                    title: oRound.title,
                                    start_at: oRound.start_at,
                                    end_at: oRound.end_at,
                                    state: oRound.state
                                };
                                $scope.$on('xxt.tms-datepicker.change', function (event, data) {
