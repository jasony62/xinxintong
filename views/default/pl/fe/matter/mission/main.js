define(['frame'], function (ngApp) {
  ...
  ngApp.provider.controller('ctrlSetting', [
    ...
    'mediagallery',
    'srvTag',
    function ($scope, http2, $uibModal, mediagallery, srvTag) {
      ...
      $scope.remove = function () {
        if (window.confirm('确定删除项目？')) {
          http2
            .get('/rest/pl/fe/matter/mission/remove?id=' + $scope.mission.id)
            .then(function (rsp) {
        ...
        $uibModal
          .open({
            ...
            controller: [
              ...
              'mission',
              'mediagallery',
              function ($scope2, $mi, mission, mediagallery) {
                var tinymceEditor
                ...
                $scope2.$on('tinymce.instance.init', function (event, editor) {
                  ...
                  if (page) {
                    ...
                    http2
                      .get(
                        '/rest/pl/fe/matter/mission/page/create?id=' +
                          $scope.mission.id +
                          '&page=' +
                          type
                      )
          ...
          .result.then(function (result) {
            http2
              .post(
                '/rest/pl/fe/matter/mission/page/update?id=' +
                  $scope.mission.id +
                  '&page=' +
                  type,
                result
              ...
              .then(function (rsp) {
                if (data.appType === 'mschema') {
                  ...
                  http2.get(url).then(function (rsp) {
                    ...
                    if (
                      mission.userApp.data_schemas &&
                      angular.isString(mission.userApp.data_schemas)
                    ) {
                      mission.userApp.data_schemas = JSON.parse(
                        mission.userApp.data_schemas
                      )
                    }
                  })
