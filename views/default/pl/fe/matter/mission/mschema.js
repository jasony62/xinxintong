    ...
    function (
      ...
      $scope.chooseMschema = function () {
        ...
        if ((mschema = _oSelected.mschema)) {
          $scope.searchBys = []
          mschema.attr_name[0] == 0 &&
            $scope.searchBys.push({
              n: '姓名',
              v: 'name',
            })
          mschema.attr_mobile[0] == 0 &&
            $scope.searchBys.push({
              n: '手机号',
              v: 'mobile',
            })
          mschema.attr_email[0] == 0 &&
            $scope.searchBys.push({
              n: '邮箱',
              v: 'email',
            })
      ...
      $scope.createMschema = function () {
        ...
        if ($scope.mission && $scope.mission.siteid) {
          url =
            '/rest/pl/fe/site/member/schema/create?site=' +
            $scope.mission.siteid
          proto = {
            valid: 'Y',
            matter_id: $scope.mission.id,
            matter_type: $scope.mission.type,
            title:
              $scope.mission.title + '-通讯录' + ($scope.mschemas.length + 1),
          }
      ...
      $scope.doSearch = function (page) {
        ...
        url =
          ...
          _oSelected.mschema.siteid +
          '&schema=' +
          _oSelected.mschema.id
        url += filter
        url += '&contain=total'
      ...
      $scope.createByAccount = function () {
        /* 选择一个访客用户 */
        tkAccount
          .pick({ id: _oSelected.mschema.siteid }, { single: true })
          .then(function (oSiteAccount) {
            if (oSiteAccount) {
              /* 访客用户创建通讯录用户 */
              tkMember
                .create(_oSelected.mschema, { userid: oSiteAccount.uid })
                .then(function (oNewMember) {
                  oNewMember._extattr = tmsSchema.member.getExtattrsUIValue(
                    _oSelected.mschema.extAttrs,
                    oNewMember
                  )
        ...
        pushnotify.open(
          ...
          function (notify) {
            ...
            if (notify.matters.length) {
              ...
              url =
                '/rest/pl/fe/site/member/notice/send?site=' +
                _oSelected.mschema.siteid
