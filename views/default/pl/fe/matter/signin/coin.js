define(['frame'], function (ngApp) {
  'use strict'
  ngApp.provider.controller('ctrlCoin', [
    '$scope',
    'http2',
    'srvSigninApp',
    function ($scope, http2, srvApp) {
      var actions = [
        {
      ...
      $scope.save = function () {
        ...
        url =
          '/rest/pl/fe/matter/signin/coin/saveRules?site=' + $scope.app.siteid
      ...
      $scope.fetchRules = function () {
        let url = `/rest/pl/fe/matter/signin/coin/rules?site=${$scope.site.id}&app=${$scope.app.id}`
        http2.get(url).then(function (rsp) {
          rsp.data.forEach(function (rule) {
            let rule2 = $scope.rules[rule.act]
            rule2.id = rule.id
      ...
      $scope.fetchLogs = function () {
        var url
        url =
          '/rest/pl/fe/matter/signin/coin/logs?site=' +
          $scope.app.siteid +
          '&app=' +
          $scope.app.id +
          page.j()
