(function() {
    ...
    xxtApp.register.controller('approveCtrl', ['$scope', 'http2', function($scope, http2) {
        ...
        $scope.approve = function(msg) {
            http2.get('/rest/pl/fe/matter/wall/message/approve?wall=' + $scope.wid + '&id=' + msg.id, function(rsp) {
                var i = $scope.messages.indexOf(msg);
                $scope.messages.splice(i, 1);
            });
        };
        $scope.reject = function(msg) {
            http2.get('/rest/pl/fe/matter/wall/message/reject?wall=' + $scope.wid + '&id=' + msg.id, function(rsp) {
                var i = $scope.messages.indexOf(msg);
                $scope.messages.splice(i, 1);
            });
        };
