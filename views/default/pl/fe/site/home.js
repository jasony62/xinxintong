define(['frame'], function (ngApp) {
  'use strict'
  ngApp.provider.controller('ctrlHome', [
    '$scope',
    'http2',
    'noticebox',
    'mediagallery',
    function ($scope, http2, noticebox, mediagallery) {
      ...
      $scope.editPage = function (page) {
        ...
        if (name && name.length) {
          location.href =
            '/rest/pl/fe/code?site=' + $scope.site.id + '&name=' + name
        } else {
          http2
            .get(
              '/rest/pl/fe/site/pageCreate?site=' +
                $scope.site.id +
                '&page=' +
                page
            )
      ...
      $scope.resetPage = function (page) {
        if (window.confirm('重置操作将覆盖已经做出的修改，确定重置？')) {
          ...
          if (name && name.length) {
            http2
              .get(
                '/rest/pl/fe/site/pageReset?site=' +
                  $scope.site.id +
                  '&page=' +
                  page
              )
      ...
      $scope.update = function (name) {
        if (
          name !== 'autoup_homepage' ||
          window.confirm(
            '勾选后，如果团队主页页面有更新将会自动覆盖现有主页页面，确定自动更新？'
          )
        ) {
          var p = {}
          p[name] = $scope.site[name]
          http2
            .post('/rest/pl/fe/site/update?site=' + $scope.site.id, p)
            .then(function (rsp) {})
        }
      }
      ...
      $scope.$watch('site', function (oSite) {
        ...
        entry = {
          url: oSite.homeUrl,
          qrcode:
            '/rest/pl/fe/site/qrcode?site=' +
            oSite.id +
            '&url=' +
            encodeURIComponent(oSite.homeUrl),
        }
        $scope.entry = entry
      })
  ...
  ngApp.provider.controller('ctrlHomeCarousel', [
    ...
    'http2',
    'mediagallery',
    function ($scope, http2, mediagallery) {
      function update(name) {
        var p = {},
          site = $scope.site
        p[name] = site[name]
        http2
          .post('/rest/pl/fe/site/update?site=' + site.id, p)
          .then(function (rsp) {})
      }
      var slides
      $scope.add = function () {
  ...
  ngApp.provider.controller('ctrlHomeChannel', [
    '$scope',
    '$uibModal',
    'http2',
    'srvSite',
    'noticebox',
    function ($scope, $uibModal, http2, srvSite, noticebox) {
      ...
      $scope.edit = function (channel) {
        $uibModal
          ...
          .result.then(function (newChannel) {
            ...
            url += '?site=' + channel.siteid + '&id=' + channel.id
            http2
              .post(url, { display_name: channel.display_name })
              .then(function (rsp) {
      ...
      function updateSeq() {
        ...
        http2
          .post(
            '/rest/pl/fe/site/setting/page/seqHomeChannel?site=' +
              $scope.site.id,
            updated
          )
          .then(function (rsp) {})
      ...
      $scope.create = function () {
        http2
          .get('/rest/pl/fe/matter/channel/create?site=' + $scope.site.id)
          .then(function (rsp) {
            var channel = rsp.data
            http2
              .post(
                '/rest/pl/fe/site/setting/page/addHomeChannel?site=' +
                  $scope.site.id,
                channel
              )
              .then(function (rsp) {
      ...
      $scope.remove = function (homeChannel, index) {
        if (window.confirm('确定删除主页上的频道？')) {
          http2
            .get(
              '/rest/pl/fe/site/setting/page/removeHomeChannel?site=' +
                $scope.site.id +
                '&id=' +
                homeChannel.id
            )
      ...
      $scope.fresh = function (homeChannel) {
        http2
          .get(
            '/rest/pl/fe/site/setting/page/refreshHomeChannel?site=' +
              $scope.site.id +
              '&id=' +
              homeChannel.id
          )
      ...
      $scope.$watch('site', function (site) {
        if (site === undefined) return
        http2
          .get('/rest/pl/fe/site/setting/page/listHomeChannel?site=' + site.id)
          .then(function (rsp) {
