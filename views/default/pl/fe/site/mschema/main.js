define(['frame'], function (ngApp) {
      ...
      $scope.importMember = function () {
        $uibModal.open({
          ...
          controller: [
            ...
            '$q',
            'noticebox',
            function ($scope2, $mi, $q, noticebox) {
              http2
                .get(
                  '/rest/pl/fe/site/member/schema/listImportSchema?site=' +
                    $scope.site.id +
                    '&id=' +
                    $scope.choosedSchema.id
                )
                .then(function (rsp) {
                ...
                http2
                  ...
                  .then(function (rsp) {
                    if (rsp.data.state !== 'end') {
                      ...
                      noticebox.success(
                        '已导入用户' + rsp.data.plan + '/' + rsp.data.total
                      )
  ...
  ngApp.provider.controller('ctrlWxQrcode', [
    ...
    function ($scope, http2) {
      ...
      $scope.download = function () {
        $(
          '<a href="' + $scope.qrcode.pic + '" download="微信二维码.jpeg"></a>'
        )[0].click()
      }
      $scope.$watch('choosedSchema', function (mschema) {
        if (mschema) {
          ...
          http2
            .get(
              '/rest/pl/fe/site/member/schema/wxQrcode?site=' +
                oMschema.siteid +
                '&mschema=' +
                oMschema.id
            )
  ...
  ngApp.provider.controller('ctrlInvite', [
    '$scope',
    '$uibModal',
    'http2',
    function ($scope, $uibModal, http2) {
      function listInvite(oSchema) {
        http2
          .get('/rest/pl/fe/site/member/invite/list?schema=' + oSchema.id)
          .then(function (rsp) {
        ...
        $uibModal
          .open({
            ...
            controller: [
              ...
              function ($mi, $scope2) {
                $scope2.option = {
                  max_count: oInvite.max_count,
                  expire_at: oInvite.expire_at,
                }
                $scope2.cancel = function () {
                  $mi.dismiss()
                }
          ...
          .result.then(function (option) {
            http2
              .post(
                '/rest/pl/fe/site/member/invite/update?invite=' + oInvite.id,
                option
              )
              .then(function (rsp) {
      ...
      $scope.addInvite = function () {
        $uibModal
          ...
          .result.then(function (option) {
            http2
              .post(
                '/rest/pl/fe/site/member/invite/add?schema=' +
                  $scope.choosedSchema.id,
                option
              )
              .then(function (rsp) {
      ...
      $scope.stopInvite = function (oInvite) {
        http2
          .post('/rest/pl/fe/site/member/invite/update?invite=' + oInvite.id, {
            stop: 'Y',
          })
          .then(function (rsp) {
            angular.extend(oInvite, rsp.data)
