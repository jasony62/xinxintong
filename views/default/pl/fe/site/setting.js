define(['require'], function (require) {
  ...
  ngApp.config([
    ...
    function ($lp, $provide, $cp, $rp) {
      ...
      $rp
        .when('/rest/pl/fe/site/setting/basic', new RouteParam('basic'))
        ...
        .when(
          '/rest/pl/fe/site/setting/tmplmsg',
          new RouteParam('tmplmsg', true)
        )
  ...
  ngApp.controller('ctrlSite', [
    '$scope',
    '$location',
    'http2',
    function ($scope, $location, http2) {
      $scope.siteId = $location.search().site
      http2
        .get('/rest/pl/fe/site/get?site=' + $scope.siteId)
        .then(function (rsp) {
          $scope.site = rsp.data
        })
    },
  ])
  ngApp.controller('ctrlSetting', [
    '$scope',
    'http2',
    'mediagallery',
    'noticebox',
    function ($scope, http2, mediagallery, noticebox) {
      ...
      $scope.update = function (name) {
        ...
        http2
          .post('/rest/pl/fe/site/update?site=' + $scope.siteId, p)
          .then(function (rsp) {})
      }
      ...
      $scope.quit = function () {
        if (window.confirm('确定退出团队？')) {
          var url =
            '/rest/pl/fe/site/setting/admin/remove?site=' +
            $scope.siteId +
            '&uid=' +
            $scope.site.uid
          http2.get(url).then(function (rsp) {
      ...
      $scope.editPage = function (event, page) {
        ...
        if (name && name.length) {
          location.href =
            '/rest/pl/fe/code?site=' + $scope.siteId + '&name=' + name
        } else {
          http2
            .get(
      ...
      $scope.gotoSns = function (snsName) {
        location.href =
          '/rest/pl/fe/site/sns/' + snsName + '?site=' + $scope.siteId
      }
      /* 下面两段代码的逻辑要优化 */
  ...
  ngApp.controller('ctrlMschema', [
    '$scope',
    'http2',
    '$http',
    '$uibModal',
    'MemberSchema',
    function ($scope, http2, $http, $uibModal, MemberSchema) {
      ...
      $scope.fullUrl = function (schema) {
        var url = ''
        !/^http/.test(schema.url) &&
          (url = location.protocol + '//' + location.host)
        return (
          url + schema.url + '?site=' + $scope.siteId + '&schema=' + schema.id
        )
      }
      ...
      $scope.delSchema = function (schema) {
        var url =
          '/rest/pl/fe/site/member/schema/delete?site=' +
          $scope.siteId +
          '&id=' +
          schema.id
        http2.get(url).then(function (rsp) {
      ...
      $scope.gotoCode = function (schema) {
        if (schema.page_code_name && schema.page_code_name.length) {
          ...
          http2
            .get('/rest/pl/fe/code/create?site=' + $scope.siteId)
            .then(function (rsp) {
              var nv = {
                code_id: rsp.data.id,
      ...
      $scope.resetCode = function (schema) {
        if (schema.page_code_name && schema.page_code_name.length) {
          if (window.confirm('重置操作将覆盖已经做出的修改，确定重置？')) {
            http2
              .get(
                '/rest/pl/fe/site/member/schema/pageReset?site=' +
                  $scope.siteId +
                  '&name=' +
                  schema.page_code_name
              )
