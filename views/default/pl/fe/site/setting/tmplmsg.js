define(['main'], function (ngApp) {
  'use strict'
  ngApp.provider.service('serTmplmsg', [
    '$q',
    'http2',
    function ($q, http2) {
      ...
      this.list = function () {
        var defer = $q.defer()
        http2
          .get(_baseURL + '/list?site=' + _siteId + '&cascaded=Y')
          .then(function (rsp) {
      ...
      this.create = function () {
        var defer = $q.defer()
        http2
          .get(_baseURL + '/create?site=' + _siteId + '')
          .then(function (rsp) {
            defer.resolve(rsp.data)
          })
        return defer.promise
      ...
      this.update = function (id, data) {
        var defer = $q.defer()
        http2
          .post(_baseURL + '/update?site=' + _siteId + '&id=' + id, data)
          .then(function (rsp) {
            defer.resolve(rsp.data)
          })
        return defer.promise
      ...
      this.remove = function (id) {
        var defer = $q.defer()
        http2
          .get(_baseURL + '/remove?site=' + _siteId + '&id=' + id)
          .then(function (rsp) {
            defer.resolve(rsp.data)
          })
        return defer.promise
      ...
      this.addParam = function (tmplmsgId) {
        var defer = $q.defer()
        http2
          .get(_baseURL + '/addParam?site=' + _siteId + '&tid=' + tmplmsgId)
          .then(function (rsp) {
            defer.resolve(rsp.data)
          })
        return defer.promise
      ...
      this.updateParam = function (paramId, updated) {
        ...
        http2
          .post(
            _baseURL + '/updateParam?site=' + _siteId + '&id=' + paramId,
            updated
          )
          .then(function (rsp) {
            defer.resolve(rsp.data)
      ...
      this.removeParam = function (removed) {
        var defer = $q.defer()
        http2
          .get(_baseURL + '/removeParam?site=' + _siteId + '&pid=' + removed.id)
          .then(function (rsp) {
            defer.resolve(rsp.data)
          })
        return defer.promise
      }
      this.createMsgChannel = function () {
        var defer = $q.defer()
        http2
          .get(_baseURL + '/createMsgChannel?site=' + _siteId)
          .then(function (rsp) {
            defer.resolve(rsp.data)
          })
        return defer.promise
      ...
      this.synWx = function () {
        var defer = $q.defer()
        http2
          .get(_baseURL + '/synTemplateList?site=' + _siteId)
          .then(function (rsp) {
            defer.resolve(rsp.data)
          })
        return defer.promise
  ...
  ngApp.provider.controller('ctrlTmplmsg', [
    '$scope',
    'serTmplmsg',
    'noticebox',
    function ($scope, serTmplmsg, noticebox) {
      ...
      $scope.createMsgChannel = function () {
        serTmplmsg.createMsgChannel().then(function (data) {
          noticebox.success('完成创建')
        })
      }
  ...
  ngApp.provider.controller('ctrlSetting', [
    '$scope',
    'serTmplmsg',
    function ($scope, serTmplmsg) {
      serTmplmsg.setSiteId($scope.siteId)
      ...
      $scope.addParam = function () {
        serTmplmsg.addParam($scope.editing.id).then(function (data) {
          ...
          !angular.isArray($scope.editing.params) &&
  ...
  ngApp.provider.controller('sendCtrl', [
    '$rootScope',
    '$scope',
    'http2',
    '$uibModal',
    function ($rootScope, $scope, http2, $uibModal) {
      $scope.matterTypes = [
        {
          value: 'article',
          title: '单图文',
      ...
      $scope.startMatterPicker = function () {
        $scope.$broadcast('mattersgallery.open', function (
          aSelected,
          matterType
        ) {
          if (aSelected.length) {
  ...
  ngApp.provider.controller('logCtrl', [
    '$scope',
    'http2',
