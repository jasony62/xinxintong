define(['main'], function(ngApp) {
	...
	ngApp.provider.controller('ctrlText', ['$scope', 'http2', 'matterTypes', 'srvSite', function($scope, http2, matterTypes, srvSite) {
		...
		$scope.create = function() {
			srvSite.openGallery({
				...
				if (result.matters.length === 1) {
					...
					http2.post('/rest/pl/fe/site/sns/qy/text/create?site=' + $scope.siteId, result.matters[0], function(rsp) {
		...
		$scope.remove = function() {
			http2.get('/rest/pl/fe/site/sns/qy/text/delete?site=' + $scope.siteId + '&id=' + $scope.editing.id, function(rsp) {
		...
		$scope.edit = function(call) {
			if (call.matter === undefined) {
				http2.get('/rest/pl/fe/site/sns/qy/text/cascade?site=' + $scope.siteId + '&id=' + call.id, function(rsp) {
		...
		$scope.setReply = function() {
			srvSite.openGallery({
				...
				if (result.matters.length === 1) {
					...
					http2.post('/rest/pl/fe/site/sns/qy/text/setreply?site=' + $scope.siteId + '&id=' + $scope.editing.id, p, function(rsp) {
		...
		http2.get('/rest/pl/fe/site/sns/qy/text/list?site=' + $scope.siteId + '&cascade=n', function(rsp) {
