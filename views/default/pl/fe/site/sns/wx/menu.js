    ...
    ngApp.provider.controller('ctrlMenu', ['$scope', 'http2', 'matterTypes', '$timeout', 'srvSite', function($scope, http2, matterTypes, $timeout, srvSite) {
        ...
        $scope.edit = function(button) {
            if (button.sub === undefined || button.sub.length === 0) {
                http2.get('/rest/pl/fe/site/sns/wx/menu/get?site=' + $scope.siteId + '&k=' + button.menu_key).then(function(rsp) {
        ...
        $scope.appendButton = function(evt) {
            ...
            http2.post('/rest/pl/fe/site/sns/wx/menu/createButton?site=' + $scope.siteId, button).then(function(rsp) {
                button = rsp.data;
        ...
        $scope.appendSubButton = function(button, afterIndex) {
            ...
            http2.post('/rest/pl/fe/site/sns/wx/menu/createSubButton?site=' + $scope.siteId, subButton).then(function(rsp) {
                subButton = rsp.data;
        ...
        $scope.removeButton = function(button, index, evt) {
            evt.preventDefault();
            evt.stopPropagation();
            http2.get('/rest/pl/fe/site/sns/wx/menu/removeButton?site=' + $scope.siteId + '&k=' + button.menu_key).then(function(rsp) {
        ...
        $scope.removeSubButton = function(button, subButton, index, evt) {
            evt.preventDefault();
            evt.stopPropagation();
            http2.get('/rest/pl/fe/site/sns/wx/menu/removeButton?site=' + $scope.siteId + '&k=' + subButton.menu_key).then(function(rsp) {
        ...
        $scope.setReply = function() {
            srvSite.openGallery({
                ...
                if (result.matters.length === 1) {
                    ...
                    http2.post('/rest/pl/fe/site/sns/wx/menu/setreply?site=' + $scope.siteId + '&k=' + $scope.editing.menu_key, p).then(function(rsp) {
        ...
        $scope.update = function(name) {
            if (!angular.equals($scope.editing, $scope.persisted)) {
                ...
                http2.post('/rest/pl/fe/site/sns/wx/menu/update?site=' + $scope.siteId + '&k=' + $scope.editing.menu_key, p).then(function(rsp) {
                    if (rsp.data.menu_key) {
                        angular.extend($scope.editing, rsp.data);
                        $scope.edit($scope.editing)
                    };
        ...
        $scope.removeMenu = function() {
            if (confirm('确定删除菜单？')) {
                http2.get('/rest/pl/fe/site/sns/wx/menu/removeMenu?site=' + $scope.siteId).then(function(rsp) {
                    setPublishState('Y');
                    $scope.editing = false;
                    $scope.menu = [];
                });
        ...
        $scope.publish = function() {
            http2.get('/rest/pl/fe/site/sns/wx/menu/publish?site=' + $scope.siteId).then(function(rsp) {
                setPublishState('Y');
            });
        };
        $scope.$on('orderChanged', function(e, moved) {
            if (-1 === $scope.menu.indexOf(moved)) {
                ...
                http2.post('/rest/pl/fe/site/sns/wx/menu/setpos?site=' + $scope.siteId + '&k=' + moved.menu_key, pos).then(function() {
                    setPublishState('N');
                });
            } else {
        ...
        http2.get('/rest/pl/fe/site/sns/wx/menu/get?site=' + $scope.siteId).then(function(rsp) {
            $scope.menu = [];
