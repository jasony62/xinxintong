define(['main'], function(ngApp) {
    ...
    ngApp.provider.controller('ctrlQrcode', ['$scope', 'http2', 'matterTypes', 'srvSite', function($scope, http2, matterTypes, srvSite) {
        ...
        $scope.create = function() {
            http2.get('/rest/pl/fe/site/sns/wx/qrcode/create?site=' + $scope.siteId).then(function(rsp) {
        ...
        $scope.edit = function(call) {
            if (call && call.matter === undefined && call.matter_id && call.matter_type) {
                http2.get('/rest/pl/fe/site/sns/wx/qrcode/matter?site=' + $scope.siteId + '&id=' + call.matter_id + '&type=' + call.matter_type).then(function(rsp) {
                    var matter = rsp.data;
        ...
        $scope.setReply = function() {
            srvSite.openGallery({
                ...
                if (result.matters.length === 1) {
                    ...
                    http2.post('/rest/pl/fe/site/sns/wx/qrcode/update?site=' + $scope.siteId + '&id=' + $scope.editing.id, p).then(function(rsp) {
                        $scope.editing.matter = result.matters[0];
                    });
                }
            });
        };
        http2.get('/rest/pl/fe/site/sns/wx/qrcode/list?site=' + $scope.siteId).then(function(rsp) {
