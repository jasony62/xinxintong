    ...
    ngApp.provider.controller('ctrlPage', ['$scope', 'http2', function($scope, http2) {
        $scope.edit = function() {
            ...
            if (name && name.length) {
                ...
                http2.get('/rest/pl/fe/site/sns/yx/page/create?site=' + $scope.siteId).then(function(rsp) {
        ...
        $scope.reset = function() {
            if (window.confirm('重置操作将覆盖已经做出的修改，确定重置？')) {
                var name = $scope.yx.follow_page_name;
                if (name && name.length) {
                    http2.get('/rest/pl/fe/site/sns/yx/page/reset?site=' + $scope.siteId + '&name=' + name).then(function(rsp) {
                        location.href = '/rest/pl/fe/code?site=' + $scope.siteId + '&name=' + name;
                    });
                } else {
                    http2.get('/rest/pl/fe/site/sns/yx/page/create?site=' + $scope.siteId).then(function(rsp) {
