...
ngApp.controller('ctrlMain', [
  ...
  'http2',
  'tmsSnsShare',
  function ($scope, http2, tmsSnsShare) {
    $scope.requireLogin = false
    $scope.data = {}
    $scope.submit = function () {
      http2
        .post('/rest/i/matterUrl?invite=' + $scope.invite.id, $scope.data)
        .then(function (rsp) {
    ...
    http2.get('/rest/site/fe/user/get').then(function (rsp) {
      ...
      if (oUser.unionid && oUser.uname) {
        if (
          /^1(3[0-9]|4[57]|5[0-35-9]|7[0135678]|8[0-9])\d{8}$/.test(oUser.uname)
        ) {
          unameType = 'mobile'
        } else if (
      ...
      http2
        .get('/rest/site/fe/invite/get?inviteCode=' + inviteCode)
        .then(function (rsp) {
          ...
          if (/MicroMessenger/i.test(navigator.userAgent)) {
            ...
            tmsSnsShare.set(
              oInvite.matter_title,
              sharelink,
              oInvite.matter_summary,
              oInvite.matter_pic
            )
          }
          if (oInvite.entryRule) {
            ...
            if (oInvite.entryRule.scope === 'member') {
              if (
                oInvite.entryRule.mschemas &&
                oInvite.entryRule.mschemas.length
              ) {
