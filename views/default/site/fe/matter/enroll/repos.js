...
window.moduleAngularModules = [
  ...
  'round.ui.enroll',
  'history.ui.enroll',
  'record.repos.ui.enroll',
  'cowork.repos.ui.enroll',
  'tag.ui.enroll',
  'topic.ui.enroll',
  'assoc.ui.enroll',
  'task.ui.enroll',
  ...
  function (
    ...
    $scope.gotoTask = function (oTask) {
      if (oTask && oTask.topic && oTask.topic.id)
        location.href =
          LS.j('', 'site', 'app') + '&topic=' + oTask.topic.id + '&page=topic'
    }
    ...
    $scope.$on('xxt.app.enroll.ready', function (event, params) {
      ...
      if (
        ...
        /* 作为分类目录的题目 */
        http2
          .get(LS.j('repos/dirSchemasGet', 'site', 'app'))
          .then(function (rsp) {
          ...
          $scope.navs.forEach(function (nav) {
            if (nav.type === 'repos') {
              nav.views.forEach(function (view) {
                view.url =
                  '/views/default/site/fe/matter/enroll/home/' +
                  view.type +
                  '.html?_=1'
...
ngApp.controller('ctrlReposRecord', [
  '$scope',
  '$timeout',
  '$q',
  'http2',
  ...
  'picviewer',
  'enlAssoc',
  function ($scope, $timeout, $q, http2, noticebox, LS, picviewer, enlAssoc) {
    ...
    $scope.appendToEle = angular.element(
    ...
    $scope.dirClicked = function (oDir, active) {
      ...
      if (Object.keys(beforeData).length !== Object.keys(newData).length) {
        ...
        newData[oDir.schema_id] !== beforeData[oDir.schema_id]
      ) {
        $scope.recordList(1)
      }
    }
    ...
    $scope.confirm = function (filterOpt) {
      $scope.recordList(1).then(function () {
        http2
          .get(LS.j('repos/criteriaGet', 'site', 'app'))
          .then(function (rsp) {
            if (rsp.data) {
              var _oNew = []
    ...
    $scope.remarkRecord = function (oRecord, event) {
      ...
      if (
        ...
        target.parentNode.getAttribute('ng-click')
      )
        return
      if (
        /button/i.test(target.tagName) ||
        /button/i.test(target.parentNode.tagName)
      )
        return
    ...
    $scope.shareRecord = function (oRecord) {
      var url, shareby
      url =
        LS.j('', 'site', 'app') + '&ek=' + oRecord.enroll_key + '&page=share'
    ...
    $scope.editRecord = function (event, oRecord) {
      if (
        ...
        oRecord.userid !== $scope.user.uid &&
        $scope.user.is_editor !== 'Y'
      )
        return noticebox.warn('不允许编辑其他用户提交的记录')

      var page = $scope.app.pages.find((p) => p.type === 'I')
      if (page) $scope.gotoPage(event, page, oRecord.enroll_key)
    }
        ...
        if (at == _cPage.at) {
          $timeout(function () {
            document.documentElement.scrollTop = parseInt(
              window.sessionStorage.getItem('listStorageY')
            )
...
ngApp.controller('ctrlReposCowork', [
  '$scope',
  '$timeout',
  '$q',
  'http2',
  'tmsLocation',
  'picviewer',
  function ($scope, $timeout, $q, http2, LS, picviewer) {
    function fnGetCriteria(datas) {
      $scope.singleFilters = []
      $scope.multiFilters = []
      angular.forEach(datas, function (data, index) {
    ...
    $scope.confirm = function (filterOpt) {
      $scope.recordList(1).then(function () {
        var url =
          LS.j('repos/criteriaGet', 'site', 'app') + '&viewType=coworkData'
        http2.get(url).then(function (rsp) {
          if (rsp.data) {
            var _oNew = []
            angular.forEach(rsp.data, function (data) {
...
ngApp.controller('ctrlReposTopic', [
  '$scope',
  '$q',
  'http2',
  '$timeout',
  'tmsLocation',
  function ($scope, $q, http2, $timeout, LS) {
    function fnGetCriteria(datas) {
      $scope.singleFilters = []
      $scope.multiFilters = []
      angular.forEach(datas, function (data, index) {
    ...
    $scope.recordList = function (pageAt) {
      ...
      http2
        .post(LS.j('topic/listAll', 'site', 'app'), _oCriteria)
        .then(function (rsp) {
    ...
    $scope.gotoTopic = function (oTopic, event) {
      ...
      addToCache()
      location.href =
        LS.j('', 'site', 'app') + '&topic=' + oTopic.id + '&page=topic'
    }
    if (
      window.sessionStorage &&
      window.sessionStorage.getItem('listStorage') &&
      $scope.activeView.type === 'topic'
    ) {
      var cacheData
      cacheData = JSON.parse(window.sessionStorage.getItem('listStorage'))
      $scope.singleFilters = cacheData.singleFilters
