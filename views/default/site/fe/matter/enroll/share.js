...
ngApp.controller('ctrlShare', [
  ...
  'http2',
  'enlService',
  function ($scope, $sce, $q, LS, tmsSnsShare, http2, enlService) {
    function fnSetSnsShare(oApp, message, anchor) {
      function fnReadySnsShare() {
        ...
        if (LS.s().topic) {
          ...
          sharelink +=
            LS.j('', 'site', 'app', 'topic') + '&page=topic&shareby=' + shareid
        } else {
          target_type = 'cowork'
          target_title = oApp.title
          sharelink +=
            LS.j('', 'site', 'app', 'ek') + '&page=cowork&shareby=' + shareid
          if (anchor) {
            sharelink += '#' + anchor
          }
        }
        ...
        tmsSnsShare.set(oApp.title, sharelink, message, oApp.pic2 || oApp.pic)
      }
    ...
    $scope.shiftAuthor = function () {
      if (_oOptions.editorAsAuthor) {
        _oMessage.defaultAuthor === undefined &&
          (_oMessage.defaultAuthor = _oMessage.author)
    ...
    $scope.shiftInviter = function () {
      if (_oOptions.editorAsInviter) {
        _oMessage.defaultInviter === undefined &&
          (_oMessage.defaultInviter = _oMessage.inviter)
    ...
    $scope.$on('xxt.app.enroll.ready', function (event, params) {
      ...
      if (
        _oApp.actionRule &&
        _oApp.actionRule.role &&
        _oApp.actionRule.role.editor
      ) {
        if (
          _oApp.actionRule.role.editor.group &&
          _oApp.actionRule.role.editor.nickname
        ) {
      ...
      if (LS.s().data) {
        http2.get(LS.j('data/get', 'site', 'ek', 'data')).then(function (rsp) {
          ...
          _oMessage.author =
            _oUser.uid === oRecData.userid ? 'ta' : oRecData.nickname
        ...
        http2
          .get(LS.j('remark/get', 'site', 'remark') + '&cascaded=Y')
          .then(function (rsp) {
            var oRemark
            oRemark = rsp.data
            ...
            _oMessage.inviter = _oUser.nickname
            _oMessage.author =
              _oUser.uid === oRemark.userid ? 'ta' : oRemark.nickname
            if (oRemark.record) {
              _oMessage.relevent = '给'
              ...
              if (
                oRemark.data &&
                oRemark.data.userid !== oRemark.record.userid
              ) {
        ...
        http2
          .get(LS.j('repos/recordGet', 'site', 'app', 'ek'))
          .then(function (rsp) {
            var oRecord
            oRecord = rsp.data
            _oApp.record = rsp.data
            _oMessage.inviter = _oUser.nickname
            _oMessage.author =
              _oUser.uid === oRecord.userid ? 'ta' : oRecord.nickname
            _oMessage.object = '的记录。'
            if (
              _oUser.is_editor === 'Y' &&
              oEditor &&
              oRecord.is_editor === 'Y' &&
              _oUser.uid !== oRecord.userid
            ) {
              _oOptions.canEditorAsAuthor = true
            }
            _oDeferred.resolve(_oMessage)
        ...
        http2
          .get(LS.j('topic/get', 'site', 'app', 'topic'))
          .then(function (rsp) {
            var oTopic
            oTopic = rsp.data
            _oApp.record = rsp.data
            _oMessage.inviter = _oUser.nickname
            _oMessage.author =
              _oUser.unionid === oTopic.unionid ? 'ta' : oTopic.nickname
            _oMessage.object = '的专题。'
            if (
              _oUser.is_editor === 'Y' &&
              oEditor &&
              oTopic.is_editor === 'Y' &&
              _oUser.unionid !== oTopic.unionid
            ) {
              _oOptions.canEditorAsAuthor = true
            }
            _oDeferred.resolve(_oMessage)
