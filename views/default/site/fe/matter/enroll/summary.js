...
ngApp.controller('ctrlSummary', [
  '$scope',
  'tmsLocation',
  'http2',
  function ($scope, LS, http2) {
    $scope.activeNav = ''
...
ngApp.controller('ctrlSummaryRank', [
  '$scope',
  '$q',
  'http2',
  'enlRound',
  ...
  function ($scope, $q, http2, enlRound, $filter) {
    function list() {
      ...
      switch (oAppState.dimension) {
        case 'user':
          http2
            .post(
              `/rest/site/fe/matter/enroll/rank/userByApp?site=${oApp.siteid}&app=${oApp.id}`,
              oAppState.criteria
            )
            .then((rsp) => {
        ...
        case 'user_same_group':
          http2
            .post(
              `/rest/site/fe/matter/enroll/rank/userByApp?site=${oApp.siteid}&app=${oApp.id}&samegroup=Y`,
              oAppState.criteria
            )
            .then((rsp) => {
              defer.resolve(rsp.data)
        ...
        case 'schema':
          http2
            .post(
              `/rest/site/fe/matter/enroll/rank/schemaByApp?site=${oApp.siteid}&app=${oApp.id}&schema=${oAppState.criteria.obj}`,
              oAppState.criteria
            )
            .then((rsp) => {
              defer.resolve(rsp.data)
    ...
    $scope.setRound = function () {
      new enlRound($scope.app)
        .pick(oAppState.criteria.round)
    ...
    $scope.$watch('oApp', function (nv) {
      ...
      if ((oRankConfig = oApp.rankConfig)) {
        if (oRankConfig.schemas && oRankConfig.schemas.length)
          $scope.rankSchemas = dataSchemas.filter(function (oSchema) {
            return (
              oSchema.type === 'single' &&
              oRankConfig.schemas.indexOf(oSchema.id) !== -1
            )
          })
        if (oRankConfig.scopeSchemas && oRankConfig.scopeSchemas.length)
          $scope.scopeSchemas = dataSchemas.filter(function (oSchema) {
            return (
              ...
              oRankConfig.scopeSchemas.indexOf(oSchema.id) !== -1
            )
          })
        if (
          oRankConfig.scope ||
          ($scope.scopeSchemas && $scope.scopeSchemas.length)
        ) {
      ...
      if (window.localStorage) {
          ...
          function (nv) {
            if (nv)
              window.localStorage.setItem(
                'site.fe.matter.enroll.rank.appState',
                JSON.stringify(nv)
              )
          },
          true
        ...
        if (
          (oAppState = window.localStorage.getItem(
            'site.fe.matter.enroll.rank.appState'
          ))
        ) {
          ...
          if (!oAppState.aid || oAppState.aid !== oApp.id) {
            ...
            if (!oApp.entryRule.group.id) oAppState = null
          }
        }
      }
      if (!oAppState)
        oAppState = {
          aid: oApp.id,
          criteria: {
            orderby: oRankConfig.defaultItem
              ? oRankConfig.defaultItem
              : 'enroll',
            agreed: 'all',
            round: ['ALL'],
          },
      ...
      if (/user|user_same_group|group/.test(oAppState.criteria.obj)) {
        oAppState.dimension = oAppState.criteria.obj
      } else {
        if (
          oRankConfig.schemas &&
          oRankConfig.schemas.length &&
          oRankConfig.schemas.indexOf(oAppState.criteria.obj) !== -1
        )
          oAppState.dimension = 'schema'
        else oAppState.criteria.obj = oAppState.dimension = 'user'
      }

      new enlRound(oApp)
        .getRoundTitle(oAppState.criteria.round)
        ...
        if (oNew && oOld && oNew !== oOld) {
          switch (oNew) {
            case 'user':
              ...
              oAppState.criteria.orderby = oRankConfig.defaultItem
                ? oRankConfig.defaultItem
                : 'enroll'
              break
            case 'user_same_group':
              oAppState.dimension = 'user_same_group'
              oAppState.criteria.orderby = oRankConfig.defaultItem
                ? oRankConfig.defaultItem
                : 'enroll'
              break
...
ngApp.controller('ctrlSummaryVotes', [
  '$scope',
  '$q',
  '$timeout',
  'tmsLocation',
  'http2',
  'enlRound',
  function ($scope, $q, $timeout, LS, http2, enlRound) {
    ...
    $scope.gotoOptionLink = function (oSchema, oOption) {
      if (
        oOption.referRecord &&
        oOption.referRecord.ds &&
        oOption.referRecord.ds.ek &&
        oSchema.dsOps &&
        oSchema.dsOps.app &&
    ...
    $scope.getVotes = function () {
      ...
      http2
        .get(LS.j('schema/get', 'site', 'app') + '&rid=' + _oCriteria.rid)
        .then(function (rsp) {
              ...
              if ((oOriginalSchema = oSchemasById[oSchema.id])) {
                if (oSchema.ops && oSchema.ops.length) {
                  oSchema.ops.forEach(function (oOption) {
                    ...
                    oOption.p =
                      oSchema.sum > 0
                        ? ((oOption.c / oSchema.sum) * 100).toFixed(2)
                        : ''
                    /* 从数据来源活动，查看详情 */
                    if (
                      oOriginalSchema.dsOps &&
                      oOriginalSchema.showOpDsLink === 'Y'
                    ) {
                      oSchema.dsOps = oOriginalSchema.dsOps
                      for (
                        var i = 0, ii = oOriginalSchema.ops.length;
                        i < ii;
                        i++
                      ) {
...
ngApp.controller('ctrlSummaryMarks', [
  '$scope',
  '$q',
  '$timeout',
  '$filter',
  ...
  'http2',
  'enlRound',
  function ($scope, $q, $timeout, $filter, LS, http2, enlRound) {
    var _oApp, _facRound, _oCriteria, _oFilter
    $scope.criteria = _oCriteria = {} // 数据查询条件
    $scope.filter = _oFilter = {} // 过滤条件
    $scope.gotoOptionLink = function (oSchema) {
      if (oSchema.dsSchema && oSchema.dsSchema.app && oSchema.dsSchema.app.id) {
        if (
          oSchema.referRecord &&
          oSchema.referRecord.ds &&
          oSchema.referRecord.ds.ek
        ) {
          location.href =
            LS.j('', 'site') +
            '&app=' +
            oSchema.dsSchema.app.id +
            '&ek=' +
            oSchema.referRecord.ds.ek +
            '&page=cowork'
        }
          ...
          http2.get(url).then(function (rsp) {
            ...
            angular.forEach(rsp.data, function (oStat, schemaId) {
              ...
              if ((oSchema = oSchemasById[schemaId])) {
                oSchema._score = {
                  sum: oStat.sum,
                  avg: $filter('number')(oStat.sum / oStat.count, 2).replace(
                    '.00',
                    ''
                  ),
                }
                if (oSchema.ops && oSchema.ops.length) {
                  oSchema.ops.forEach(function (oOption) {
                    ...
                    if ((opSum = oStat[oOption.v])) {
                      oOption._score = {
                        sum: opSum,
                        avg: $filter('number')(opSum / oStat.count, 2).replace(
                          '.00',
                          ''
                        ),
                      }
...
ngApp.controller('ctrlSummaryStat', [
  '$scope',
  '$timeout',
  '$uibModal',
  '$q',
  ...
  function (
    ...
    $scope.shiftRound = function (oRound) {
      location.href =
        LS.j('', 'site', 'app') + '&rid=' + oRound.rid + '&page=stat'
    }
    $scope.$watch('app', function (oApp) {
      if (!oApp) {
        return
      ...
      function fnDrawChart() {
        ...
        for (var p in oStat) {
          ...
          if (/single/.test(item._schema.type)) {
            ...
            if (item.ops.length) {
              ...
              // 添加题目平均分
              avgScore = parseFloat(
                new Number(totalScore / item.ops.length).toFixed(2)
              )
              item.ops.push({
                l: '本项平均分',
                c: avgScore,
        ...
        if (scoreSummary.length) {
          avgScoreSummary = parseFloat(
            new Number(totalScoreSummary / scoreSummary.length).toFixed(2)
          )
          scoreSummary.push({
            l: '所有打分项总平均分',
            c: avgScoreSummary,
      ...
      var rpSchemas = [],
        oStat = {},
        rpConfig =
          _oApp.rpConfig && _oApp.rpConfig.op ? _oApp.rpConfig.op : undefined,
      ...
      _facRound
        .get([LS.s().rid ? LS.s().rid : _oApp.appRound.rid])
        .then(function (aRounds) {
          ...
          http2
            .get(
              LS.j('stat/get', 'site', 'app') + '&rid=' + _oCriteria.round.rid
            )
            .then(function (rsp) {
                    ...
                    if ((oStatBySchema = rsp.data[oSchema.id])) {
                      ...
                      if (oStatBySchema.ops && oStatBySchema.sum > 0) {
                        oStatBySchema.ops.forEach(function (oDataByOp) {
                          oDataByOp.p =
                            new Number(
                              (oDataByOp.c / oStatBySchema.sum) * 100
                            ).toFixed(2) + '%'
                        })
                      }
                    }
