...
ngApp.controller('ctrlTopic', [
  ...
  'noticebox',
  'enlTask',
  function ($scope, $q, http2, LS, $timeout, picviewer, noticebox, enlTask) {
    /* 是否可以对记录进行表态 */
    ...
    $scope.shareTopic = function () {
      var url, shareby
      url =
        LS.j('', 'site', 'app') + '&topic=' + $scope.topic.id + '&page=share'
      if (_shareby) {
    ...
    $scope.quitTopic = function (oRecord) {
      http2
        .post(LS.j('topic/removeRec', 'site') + '&topic=' + $scope.topic.id, {
    ...
    _shareby = location.search.match(/shareby=([^&]*)/)
          ...
          if (oTask.config_type === 'vote' && oTask.state === 'IP') {
            ...
            oTask.submit = function () {
              if (oTask.pendingVotes.length)
                http2
                  .post(
                    LS.j('task/batchVote', 'site', 'app') + '&task=' + oTask.id,
                    oTask.pendingVotes
                  )
                  .then(function (rsp) {
            ...
            oTask.tip = function () {
              if (oTask.performance) {
                if (oTask.limit) {
                  if (
                    oTask.limit.min &&
                    oTask.performance.voteNum < oTask.limit.min
                  )
                    return (
                      '还差' +
                      (oTask.limit.min - oTask.performance.voteNum) +
                      '票'
                    )
                  if (
                    oTask.limit.max &&
                    oTask.performance.voteNum > oTask.limit.max
                  )
                    return (
                      '超出' +
                      (oTask.performance.voteNum - oTask.limit.max) +
            ...
            oTask.canSubmit = function () {
              ...
              var bCanSubmit = true
              if (oTask.pendingVotes.length === 0) bCanSubmit = false
            ...
            http2
              .get(
                LS.j('task/votePerformance', 'site', 'app') +
                  '&task=' +
                  $scope.topic.task.id
              )
              .then(function (rsp) {
                ...
                oVotePerf.voteNum = oVotePerf.data_ids
                  ? oVotePerf.data_ids.length
                  : 0
              })
          }
          $scope.task = oTask
