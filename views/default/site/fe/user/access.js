  ...
  function ($scope, $http) {
    ...
    if (window.localStorage) {
      ...
      if (window.localStorage.getItem('xxt.login.gotoConsole') === 'Y') {
        $scope.loginData.gotoConsole = window.localStorage.getItem(
          'xxt.login.gotoConsole'
        )
      }
    } else {
    ...
    $scope.login = function () {
      if ($scope.loginData.password) {
        $http
          .post('/rest/site/fe/user/login/do?site=' + _siteId, $scope.loginData)
              ...
              .success(function (rsp2) {
                ...
                if (
                  ...
                  if ((oStorage = window.localStorage)) {
                    ...
                    oStorage.setItem(
                      'xxt.pl.protect.event.trace',
                      JSON.stringify(oCached)
                    )
                  }
                }
          ...
          .error(function (data, header, config, status) {
            if (data) {
              $http.post('/rest/log/add', {
                src: 'site.fe.user.login',
    ...
    $scope.logout = function () {
      $http
        .get('/rest/site/fe/user/logout/do?site=' + _siteId)
        .success(function (rsp) {
          if (rsp.err_code != 0) {
            $scope.errmsg = rsp.err_msg
            return
    ...
    $scope.register = function () {
      $http
        .post('/rest/site/fe/user/register/do?site=' + _siteId, {
    ...
    $scope.refreshPin = function () {
      var time = new Date().getTime(),
        url =
          '/rest/site/fe/user/login/getCaptcha?site=platform&codelen=4&width=130&height=34&fontsize=20'
      $scope.pinImg = url + '&_=' + time
    }
    $http.get('/rest/site/fe/get?site=' + _siteId).success(function (rsp) {
      $scope.site = rsp.data
      $http
        .get('/rest/site/fe/user/get?site=' + _siteId)
        .success(function (rsp) {
