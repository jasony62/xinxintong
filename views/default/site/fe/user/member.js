      ...
      controller: [
        ...
        function ($scope, $timeout, noticebox) {
          $scope.chooseImage = function (oSchema) {
            ...
            if (oSchema !== null) {
              aModifiedImgFields.indexOf(oSchema.id) === -1 &&
                aModifiedImgFields.push(oSchema.id)
            ...
            window.xxt.image.choose($q.defer()).then(function (imgs) {
              ...
              $timeout(function () {
                ...
                for (i = 0, j = imgs.length; i < j; i++) {
                  img = imgs[i]
                  eleImg = document.querySelector(
                    'ul[name="' + oSchema.id + '"] li:nth-last-child(2) img'
                  )
                  if (eleImg) {
...
ngApp.controller('ctrlMember', [
  ...
  'http2',
  'tmsSchema',
  function ($parse, $scope, $timeout, noticebox, LS, http2, tmsSchema) {
    /**
     * 控制关联题目的可见性
     */
            ...
            oSchema.ops.forEach((oOption) => {
              ...
              if (oOption.g && oOption.g === oOptGroup.i) {
                if (
                  oSchema.type === 'single' &&
                  oSchema.pageConfig &&
                  oSchema.pageConfig.component === 'S'
                ) {
                  domOption = document.querySelector(
                    'option[name="' + oSchema.id + '"][value=' + oOption.v + ']'
                  )
                  ...
                  if (oSchema.type === 'single') {
                    domOption = document.querySelector(
                      'input[name=' + oSchema.id + '][value=' + oOption.v + ']'
                    )
                  } else if (oSchema.type === 'multiple') {
                    domOption = document.querySelector(
                      'input[ng-model="' + oSchema.id + '.' + oOption.v + '"]'
                    )
                  }
                  if (
                    domOption &&
              ...
              if (oOption.g && oOption.g === oOptGroup.i) {
                if (
                  ...
                  domSelect = document.querySelector(
                    '[schema="' + oSchema.id + '"] select[ng-model]'
                  )
                  if (domSelect) {
    ...
    function fnValidate(oSchema) {
      ...
      if (
        member.name &&
        false === required(member.name, 2, '请提供您的姓名！')
      ) {
        return false
      }
      if (
        member.mobile &&
        false === isMobile(member.mobile, '请提供正确的手机号（11位数字）！')
      ) {
        return false
      }
      if (
        member.email &&
        false === isEmail(member.email, '请提供正确的邮箱！')
      ) {
        return false
      }
      if (oSchema.extAttrs && oSchema.extAttrs.length) {
        ...
        for (var i = 0, ii = oSchema.extAttrs.length; i < ii; i++) {
          ...
          if (
            !oExtAttr.visibility ||
            !oExtAttr.visibility.rules ||
            oExtAttr.visibility.rules.length === 0 ||
            oExtAttr.visibility.visible
          ) {
            if (oExtAttr.required && oExtAttr.required === 'Y') {
              sCheckResult = tmsSchema.checkValue(
                oExtAttr,
                oExtValue[oExtAttr.id]
              )
      ...
      http2
        ...
        .then(
          function (rsp) {
            ...
            $scope.posting = false
            noticebox
              .confirm('已经提交成功，离开页面！', actions)
              .then(function (action) {
                if (action === 'continue') {
                  http2
                    .get(LS.j('passed', 'site', 'schema') + '&redirect=N')
                    .then(function (rsp) {
    ...
    function setMember(user) {
      ...
      if (oMember) {
        ...
        if (
          !oAttrs.mobile.hide &&
          /^1[3|4|5|6|7|8|9][0-9]\d{4,8}$/.test(user.login.uname)
        ) {
    ...
    $scope.login = function () {
      if ($scope.loginUser.password) {
        http2
          .post(
            '/rest/site/fe/user/login/do?site=' + LS.s().site,
            $scope.loginUser
          )
          .then(function (rsp) {
            http2
              .post('/rest/site/fe/user/login/checkPwdStrength', {
    ...
    $scope.logout = function () {
      http2
        .post(
          '/rest/site/fe/user/logout/do?site=' + LS.s().site,
          $scope.loginUser
        )
        .then(function (rsp) {
          location.reload(true)
    ...
    $scope.register = function () {
      http2
        .post('/rest/site/fe/user/register/do?site=' + LS.s().site, {
    ...
    $scope.refreshPin = function (preEle) {
      ...
      if (preEle) {
        ...
        pinWidth = 120
        url =
          '/rest/site/fe/user/login/getCaptcha?site=platform&codelen=4&width=' +
          pinWidth +
    ...
    http2.get('/rest/site/fe/get?site=' + LS.s().site).then(function (rsp) {
      ...
      http2
        .get(
          '/rest/site/fe/user/memberschema/get?site=' +
            LS.s().site +
            '&schema=' +
            LS.s().schema +
            '&matter=' +
        ...
        .then(function (rsp) {
          ...
          http2.get(LS.j('get', 'site', 'schema')).then(function (rsp2) {
            ...
            $timeout(function () {
              ...
              // 控制关联题目的可见性
              fnToggleAssocSchemas(
                $scope.schema.extAttrs,
                $scope.member.extattr
              )
              // 控制题目关联选项的可见性
              fnToggleAssocOptions(
                $scope.schema.extAttrs,
                $scope.member.extattr
              )
            })
            /* 解决多个注册账号的问题 */
            http2
              .get('/rest/site/fe/user/get?site=' + LS.s().site)
              .then(function (rsp) {
