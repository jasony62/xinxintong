    ...
    function ($q, http2) {
      return {
        getSiteUser: function (siteId) {
          ...
          http2
            .get('/rest/site/fe/user/get?site=' + siteId)
            .then(function (rsp) {
...
ngApp.config([
  '$controllerProvider',
  '$uibTooltipProvider',
  function ($cp, $uibTooltipProvider) {
    ngApp.provider = {
...
ngApp.controller('ctrlMain', [
  ...
  'tmsSubscribe',
  'tmsContribute',
  function ($scope, http2, srvUser, tmsDynaPage, tmsSubscribe, tmsContribute) {
    var oUser,
      ls = location.search,
    ...
    http2.get('/rest/site/home/get?site=' + siteId).then(function (rsp) {
      let oSite = rsp.data
      let { config, home_page, icp_beian } = oSite

      if (config.header && config.header.hide === 'Y') {
        $scope.showHeader = false
      } else {
      ...
      tmsDynaPage.loadCode(ngApp, home_page).then(function () {
        if (!oSite.heading_pic) {
          oSite.heading_pic = '/static/img/avatar.png'
        }
        $scope.site = oSite
        $scope.page = oSite.home_page
      })

      if (icp_beian) {
        let elemIcpbeian = document.querySelector('#icpbeian')
        if (elemIcpbeian) {
          elemIcpbeian.innerHTML = oSite.icp_beian
        }
      }
    })
  },
