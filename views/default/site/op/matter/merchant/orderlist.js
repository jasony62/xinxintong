define(["require", "angular", "base", "directive"], function(require, angular, ngApp) {
    ...
    ngApp.controller('ctrl', ['$scope', '$http', '$timeout', '$q', function($scope, $http, $timeout, $q) {
        ...
        $scope.errmsg = '';
        $http.get('/rest/site/op/matter/merchant/orderlist/pageGet?site=' + $scope.siteId + '&shop=' + $scope.shopId).success(function(rsp) {
            if (rsp.err_code !== 0) {
        ...
        $scope.open = function(order) {
            location.href = '/rest/site/op/matter/merchant/order?site=' + $scope.siteId + '&shop=' + $scope.shopId + '&order=' + order.id;
        };
    }]);
