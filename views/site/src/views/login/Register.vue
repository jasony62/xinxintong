<template>
  <register :schema="schema" :fn-captcha="fnCaptcha" :fn-register="fnRegister" :on-success="fnSuccess"
    :on-fail="fnFail">
  </register>
</template>
...
const fnRegister = async (loginData: { [k: string]: string }): Promise<any> => {
  const url = 'http://localhost:8000/rest/site/fe/user/register/do?site=platform'
